...
A {\em block-variable-declaration} allocates memory for its variable
...
are {\tt *WRITE-ONLY*} in {\em statements}
...
are initialized by the {\em allocation-call}, but the memory
pointed at is zeroed initially, 
{\tt *WRITE-ONLY*} in {\em statements}
of the {\em assignment-statement}, and 
subject to the {\em pointer-variable} {\em qualifiers} after
the {\em assignment-statement} finished execution.
...
Declarations containing an {\em allocation-call} set the memory, which
is a pointer variable, to the result of the {\em allocation-call}
(see below), which in turn points at allocated memory that is zeroed.
Other declarations zero the variable's memory if they are not
{\em next-declarations},
and set the variable's memory
to the previous value of the declaration's variable if
the declarations are {\em next-declarations}.
...
are write-only, and after this execution, these variables become
{\tt co}.  During this execution variables with
an {\em allocation-call} are {\tt co} but the memory they point
at is write-only, and after execution this memory becomes whatever
its pointer qualifiers specify.

When a declaration has an {\em allocation-call}, its variable must have pointer
