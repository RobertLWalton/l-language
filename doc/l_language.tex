...
variables whose pointer type has the {\tt *LOCAL*} qualifier.
...
restrictive rule is simply $d2 < d1$.  However the compiler
cannot enforce this rule; to use the rule it would be necessary
...
by a `{\tt @= local \ldots}' {\em allocation-call}\pagnote{ALLOCATION-CALL}
is the depth of the statement that assigns the variable.
The lifetime depth of the value of an assignment variable or inline prototype
argument or result variable is the lifetime depth of the value assigned
to the variable.  Note that in the case that the pointer assigned is
{\tt *GLOBAL*}, this depth is 0.
...
\\[1ex]
		& Note: If an inline function is called from a {\em statement}
		  ...
		  rule (L6) will give the same answers as it would if
...
An example involving inline functions is:
\begin{indpar}\begin{verbatim}
type list element:
