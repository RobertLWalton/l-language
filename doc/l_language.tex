...
it declares typically has with exactly $N$ `\TT{@}'s after
any initial `\TT{.}'s.
It may have fewer `\TT{@}'s, including none, but may not have more.
\item
...
When a field is declared, the current offset is incremented by the
...
{\em field-label} or {\em subfield-lable} declared by the
