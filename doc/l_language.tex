...
The marjority of operators map onto functions.  For example, \\
...
See Conditional Statments\pagnote{CONDITIONAL-STATEMENTS}.

\item[prefix \TT{while}]
\item[prefix \TT{until}] \vspace*{-0.15in} ~\\
See Loop Assignment Statments\pagnote{LOOP-ASSIGNMENT-STATEMENTS}.

\item[infix \TT{=}] ~\\
See Assignment Statments\pagnote{ASSIGNMENT-STATEMENTS}.
...
\item[infix \TT{-{}-{}-{}>}] ~\\
See Abbreviation Statments\pagnote{ABBREVIATION-STATEMENT}.
...
evaluates to {\tt true}.  Then the cooresponding {\em $T$-expression}
...
{\tt function const r1, const r2 = std \ttkey{floor} (const n1, const n2 )} \\
{\tt function const r1, const r2 = std \ttkey{ceiling} (const n1, const n2 )} \\
{\tt function const r1, const r2 = std \ttkey{truncate}
			(const n1, const n2 )} \\
{\tt function const r1, const r2 = std \ttkey{round} (const n1, const n2 )}
\begin{indpar}
...
{\tt N2} are identical.
\item[\tt bool ( N1 v )] ~\\
    This is defined only when {\tt N1} and {\tt N2} are identical.
\end{itemlist}

Unchecked explict conversion from values of number type {\tt N1} to values
of number type {\tt N2} are defined as follows:
\begin{itemlist}[0.5in]
\item[\tt function int\ldots{}~r = std *UNCHECKED* ( N1 v )]
\item[\tt function uns\ldots{}~r = std *UNCHECKED* ( N1 v )] \vspace*{-0.1in}
\item[\tt function bool r = std *UNCHECKED* ( N1 v )] \vspace*{-0.1in} ~\\
