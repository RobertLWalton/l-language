...
begins with a {\em variable-name}
...
itself (or by itself inside parentheses)
is called \key{compound} and consists of two parts:
a prefix that is a smaller {\em reference-expression} and a suffix that
begins with the \TT{[} or {\em member-name}.  The prefix is may be
parenthesized, so a {\em reference-expression} always begins zero or more
left parentheses followed by a {\em variable-name}.
The suffix begins with a `\TT{[}' or a {\em member-name},
and may contain parenthesized argument lists.
