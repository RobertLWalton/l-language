...
A \emkey{reference-expression} computes a pointer to a location.
In some cases it is possible to use a {\em reference-expression}
to compute a pointer that can be stored in a variable.
In other cases the pointer is virtual and
...
\emkey{reference-expression}\label{REFERENCE-EXPRESSION}
    ...
    $|$ & {\em reference-expression} {\em reference-offset} \\
    \end{tabular}
\\[0.5ex]
...
\emkey{reference-offset}\label{REFERENCE-OFFSET}
    ::= {\em member-name} $|$ \{ \TT{[} {\em index-list} \TT{]} \}\PLUS{}
\\[0.5ex]
...
Examples of virtual pointers are:
...
my type Z:       // A virtual pointer is computed to the variable Z.
    Z.X = 5      // A virtual pointer to Z.X is computed and dereferenced.
    Z.@Y = null  // A virtual pointer to Z.@Y is computed and dereferenced.
                 // This last is virtual both because it points into the
...
ap my type @W:   // A real pointer @W is computed to the variable W.
    W.X = 5      // A real pointer to W.X (denotable by @W.X) is computed
                 // and dereferenced.
    W.@Y = null  // A virtual pointer to W.@Y is computed and dereferenced.
...
These are invoked by {\em function-variable-names} that are syntactically
...
These are {\em reference-offsets} which act like postfix operators on
real reference pointers produced by base {\em reference-expressions}.
\end{itemize}
...
{\em reference-expression} that is annotated with
the right-side {\em expression's} target type.
...
are annotated together with the list of the {\em function-call's} target types.
...
search and applies the Call-Declaration Matching Algorithm (below) to each.
...
algorithm matching a {\em reference-expression} to a
...
A reference function can be invoked by a {\em function-variable-name},
that acts like a {\em variable-name} except that instead of
naming a location in the stack the {\em function-variable-name}
names a pair of functions: one for loading the value of the function variable
and one for storing it.

A reference function can also be invoked by a {\em reference-call}
that invokes functions associated with a particular {\em reference-offset},
either a {\em member-name} or \TT{[~]} bracketed expressions.
The part of the {\em reference-call} offset
is a smaller {\em reference-expression} called the {\em reference-base}.
The functions effectively apply the {\em reference-offset} as if it
were a postfix operator to the {\em reference-base} in order to
load a value, store a value, or index a {\em reference-base} pointer.
...
\\[0.5ex]
{\em reference-offset}\begin{tabular}[t]{rl}
	::= & {\em member-name} \\
	$|$ & {\em square-bracketed-call-argument-list}\PLUS{}
	\end{tabular}
\\[0.5ex]
{\em reference-base} ::= {\em reference-expression}
\\[0.5ex]
...
\begin{itemize}
\item
If the {\em reference-call} begins with a {\em module-abbreviation}
that is part of the the {\em variable-name} that is the root of
the {\em reference-expression},
\item
A {\em member-name} in a {\em reference-offset}
...
\item
The functions associated with a {\em function-variable-name} are
invoked from a
{\em simple-reference-expression}\pagnote{SIMPLE-REFERENCE-EXPRESSION}
consisting of nothing but the {\em function-variable-name}.
\item
A {\em function-variable-name} begins with a {\em module-abbreviation}
if it is external (just like other {\em variable-names}.
\item
A a {\em reference-function-declaration} \underline{not} containing
...
smaller {\em reference-expression} that has already been delimited.
\end{itemize}
...
its {\em member-name} or the number of its
{\em square-bracketed-pattern-argument-lists}.
\end{itemize}
...
\item[Step \ref{CALL-DECLARATION-REQUIRED-VALUES}%
           \pagnote{CALL-DECLARATION-REQUIRED-VALUES}]
This step is skipped completely; there are no {\em required-values}
in a {\em reference-function-declaration}.

\item[Step \ref{CALL-DECLARATION-ARGUMENT-TYPING}%
           \pagnote{CALL-DECLARATION-ARGUMENT-TYPING}]

Implicit conversion of the {\em reference-base}
is \underline{not} permitted; its type must exactly match the type
of the {\em reference-pattern-base}.
