...
\key{local} &
	...
	of an {\em assignment-state\-ment},
	or by a
	{\em prototype-result-variable-declaration} or
...
\\[1ex]
{\bf (none)}		& The compiler assigns a lifetime depth to the
                          target variables of pointers with no lifetime
			  qualifiers.  This is the lexical
			  depth of the statement that created the pointer,
			  with top level statements begin given depth 1
			  (details are below).
\\[1ex]
			& Or the compiler may assign the special depth 0,
			  which means that
			  the only thing known about the pointer value is
			  that the lifetime of its target includes the
			  lifetime of the variable in which the pointer
			  value is stored.
\end{tabular}
\end{indpar}
...
The first of these rules are:
\begin{indpar}[0.2in]
\begin{tabular}{p{0.4in}p{5.0in}}
\key{(L1)}	& A pointer value with the {\tt *GLOBAL*} qualifier
                  can be stored in any variable whose type is a
		  pointer type with the {\tt *GLOBAL*} qualifier.
\end{tabular}

\begin{tabular}{p{0.4in}p{5.0in}}
\key{(L2)}	& A pointer value with the {\tt *HEAP*} qualifier
                  can be stored in any variable whose type is a
		  pointer type with the {\tt *HEAP*} qualifier.
\end{tabular}

\begin{tabular}{p{0.4in}p{5.0in}}
\key{(L3)}	& A pointer value with the {\tt *GLOBAL*} qualifier
                  can be stored in any variable whose type is a
		  pointer type with the {\tt *HEAP*} qualifier.
\\[1ex]
