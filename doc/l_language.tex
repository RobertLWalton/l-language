...
the {\em member-name} if any, and the number and types of any {\em indices}.
\end{itemize}
...
\item A \key{declaration}.  May be (usually is) missing.  If present
this is the declaration of a reference function that must be used to 
load a value from or store a value into the target of the pointer part.
\end{enumerate}
...
Values are stored into the location specified by the reference pointer.
