...
\ttkey{L} & excludes \underline{all but} \TT{*GLOBAL*} ~and \TT{*HEAP*} \\
\end{tabular}
\end{indpar}
...
{\tt function P\$ ro QL\$ PST\$1 @r =
	std \ttkey{ro} ( P\$ QR\$ PST\$2 @p )} \\
{\tt function P\$ *WRITE-ONLY* QL\$ PST\$1 @r = \\
\hspace*{1in}std \ttkey{*WRITE-ONLY*} ( P\$ QW\$ PST\$2 @p )}
\begin{indpar}
These just copy the pointer changing its target access qualifier to {\tt ro}
or {\tt *WRITE-ONLY*} (but note that an {\tt *INIT*} qualifier cannot be changed
to {\tt *WRITE-ONLY*}).
\end{indpar}
