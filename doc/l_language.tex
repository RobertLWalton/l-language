...
the {\em prototype-pattern} is quoted (with {\tt "}), the
...
one of the following must be found: (1) a {\em function-term-name}, or
(2) the word `{\tt no}', or (3) the word `{\tt not}', or
(4) a bracketed argument list (with (~) or [~] brackets, or with
implicit brackets).
...
possibly preceeded by `{\tt no}' or `{\tt not}'.
