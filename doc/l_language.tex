...
\emkey{map-constant}\label{MAP-CONSTANT}
    ...
    \end{tabular}
\\[0.5ex]
\emkey{map-list} ::= {\em map-element} \{ \TT{,} {\em map-element} \}\STAR{}
\\[0.5ex]
\emkey{map-dictionary} ::= {\em map-attribute}
                              \{ \TT{,} {\em map-attribute} \}\STAR{}
\\[0.5ex]
\emkey{map-attribute} ::=
    {\em map-label} \TT{=>} {\em map-attribute-value}
\\[0.5ex]
\emkey{map-element}\label{MAP-ELEMENT} ::= {\em constant-expression}
\\[0.5ex]
\emkey{map-label}\label{MAP-LABEL} ::= {\em constant-expression}
\\[0.5ex]
\emkey{map-attribute-value}\label{MAP-ATTRIBUTE-VALUE}
