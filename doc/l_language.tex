...
a {\em qualifier-name}, {\em module-name}, {\em pointer-type-name}
or {\em type-name} in the sequence.
...
or subfield of this container, and the container is not a {\tt const}
value, both the container and the field or
...
and lifetime depth as the container (see \itemref{LIFETIMES}).
\end{itemize}
\end{minipage}
\end{center}
...
\emkey{dimension-size} ::=  {\em const-expression}
...
\emkey{bit-range}
    ...
    \end{tabular} \\
\emkey{onlybit} ::= {\tt const-expression}
		    with non-negative integer value \\
\emkey{highlowbits} :::= {\em dit}+ \TT{-} {\em dit}+
           ~~~~~ [this is a single lexeme] \\
\emkey{highbit} ::= {\tt const-expression}
		    with non-negative integer value \\
\emkey{lowbit} ::= {\tt const-expression}
		   with non-negative integer value \\
{\em dit} ::= see \pagref{DIT}
\\[2ex]
\emkey{alignment} ::= {\tt const-expression}
...
defining {\em type-declaration} X and within other {\em type-declarations}
...
type the given pointer type, and an explicit conversion
...
and will otherwise be taken to be simply $e$.
...
{\em default-value} is used, it is evaluated as a {\em const-expression}.
...
This section applies to matching non-{\em const-expressions} and
non-{\em reference-calls}.  The
rules of this section are modified when matching {\em const-expressions}
assording to section \itemref{CONST-EXPRESSIONS} and
{\em reference-calls}
according to section \itemref{REFERENCE-CALL-PROTOTYPE-MATCHING}.
Both {\em const-expression} and {\em reference-call} analysis
is a bottom-up process that must be done \underline{before} applying the
rules of this section.  In particular, this section assumes that
all {\em const-expressions} have been replaced by {\tt const} value,
and that the types of all {\em reference-expressions} are know.
...
If the subexpression is a {\em reference-expression}
value $e$ whose type is not exactly the target-type,
...
\end{verbatim}\end{indpar}
The right sides of these statements are {\em const-expressions} that
evaluate to {\tt 4.0} and {\tt 4.5} respectively
before the rules of this section are applied.  These {\tt const}
values are reference expressions and therefore the statements become:
\begin{indpar}\begin{verbatim}
...
compile error because it cannot convert {\tt const} value {\tt 4.5}
to an {\tt int} value.

Note that matching maps are \underline{not} used in the modified versions
of this section's algorithm that are used for {\em const-expressions}
and {\em reference-expressions}.
...
Note that a constant that replaces a {\em const-expression}
is itself a {\em reference-expression} of type {\tt const}.
...
its matching {\em actual-argument} must be a {\em const-expression} with
...
Call-prototype matching for macro functions ignores any target type,
...
If call-prototype matching for a {\em function-call} matches the
...
{\em function-call} then undergoes call-prototype matching as if
...
or a {\em function-call} such that:
\begin{enumerate}
\item All call arguments are {\tt const} constant values
(after replacement of {\em const-subexpressions} by their
values).
\item An unambiguous function prototype is found matching
the {\em function-call},
using the algorithm of \itemref{INLINE-CALL-PROTOTYPE-MATCHING},
where the only prototypes considered are those for which:
\begin{enumerate}
\item all prototype arguments and results have the {\tt const} type
\item if the {\em function-call} to be matched is \underline{not}
      the right side of
      a {\em call-assignment-statement}\pagnote{CALL-ASSIGNMENT-STATEMENTS},
      there is at least one result, and the first result will be
      the value of the {\em function-call}
\item if the {\em function-call} to be matched \underline{is} the right side
      of a {\em call-assignment-statement}, the prototype has at least
      as many prototype results as the {\em call-assignment-statement} has
      {\em assignment-results}
\end{enumerate}
\end{enumerate}

When a {\em const-expression} is found by the bottom-up
transversal, it is immediately evaluated at compile time.
If the {\em const-expression} is \underline{not} the right side
of a {\em call-assigment-statement}, its value replaces the
{\em const-expression}.
If the {\em const-expression} \underline{is} the right side
of a {\em call-assigment-statement}, the {\em call-assignment-statement}
is converted to an {\em expression-assignment-statement} with a list of
the proper number of {\tt const} results of the {\em const-expression}
becoming the {\em expression-list}.

It is a compile error if a function called during this
evaluation attempts to execute run-time code: e.g., if
it attempts to allocate a variable to the run-time stack.
The function may, however, use the {\tt *INCLUSION*}
variable (see \itemref{INCLUSIONS})
to generate run-time code that will be compiled
outside the context of the function execution - this code
will be compiled immediately after code generated by compiling the
statement containing the {\em const-expression}.
...
\hspace*{1em}\begin{tabular}[t]{rl}
    ...
    \end{tabular}
\\[0.5ex]
{\em reerence-expression} ::= see \pagref{REFERENCE-EXPRESSION}
\\[0.5ex]
...
reference function ap Q$1 flt64 @x =
    ...
    av Q$1 flt64 @p = *UNCHECKED*
        ...
        // violate type checking.  Here it takes the ap @v,
        // adds v.offset to its offset, and returns this as
        // an av with bounds 0 and v.length.
    @x = @p[index]
...
are explict brackets.  For example,
\begin{indpar}
...
If two {\em type-declarations} have overlapping scope,
they must follow the rules of \itemref{DEFINED-TYPE-EXPANSIONS}.
...
any ancestor {\em type-declaration} in the expansion.  A name declared by a
a {\em type-subdeclaration} is only visible within the
{\em type-declarations} in the same expansion tree, unless
special syntax is used to pair the name with the name of
the defined type declared by that expansion tree (e.g.,
{\em member-names} in {\em reference-expressions}).
...
\\[1ex]
		& Note: If an inline function is called from a {\em statement}
		  ...
		  rule (L5) will give the same answers as it would if
...
possibilities, while the lifetime of a target must be at least
...
\key{(V6)}	& Argument variables in an \underline{out-of-line}
		  ...
		  may be assigned {\tt *LOCAL*} pointer actual argument
		  values.
\end{tabular}
\\
\begin{tabular}{lp{5.0in}}
...
\\
\begin{tabular}{lp{5.0in}}
\key{(V8)}	& A {\tt *LOCAL*} pointer stored in a variable of depth $D$ is
                  assigned depth $D$, and any {\tt *LOCAL*} variables in its
...
Note: It is a compile error if a {\tt *GLOBAL*} or {\tt *HEAP*}
pointer has a target type that has a field that is a {\tt *LOCAL*} pointer,
as per (L4).  So the converted {\tt *LOCAL*} pointer cannot have
any {\tt *LOCAL*} pointers stored into its target.
\end{indpar}
