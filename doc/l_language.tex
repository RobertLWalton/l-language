...
\item The {\em reference-expression} result is {\tt *VOLATILE*} if either
the container or field/subfield is.
\item The {\em reference-expression} result is {\tt *ATOMIC*} if either
the container or field/subfield is.
...
\ttkey{*LOCAL*}		& The lifetime starts when the statement allocating
                          the variable executes and stops when the smallest
			  block containing the statement terminates.
\\[1ex]
...
These rules do \underline{not} permit qualifier conversions:
...
refers to lifetime depth of values, which are defined below.
The rule is:
...
{\tt const atomc types = \{ "int", "uns", \ldots{} \}}
\begin{indpar}
...
done at the time they would be done were the program
...
or a multi-tasking system, this assumption is not correct.
...
\begin{enumerate}
\item
All read operations for the current process that would be
...
\begin{enumerate}
\item
All write operations for the current process that would be
before (or after) this operation in strict sequential program execution
are before (or after) this operation in actual optimized program execution.
\item
...
{\tt function std bool r = atomic compare and set ( ap QW\$1 A p, A vr, A vw )}
\begin{indpar}
...
\begin{enumerate}
\item
All read and write operations for the current process that would be
before (or after) this operation in strict sequential program execution
are before (or after) this operation in actual optimized program execution.
\item
