...
\\[1ex]
*CONST*			& the subexpression is a constant and the
...
Otherwise if the subexpression matching map entry for the {\tt const} prototype
is not {\tt *FAIL*} and the target type is not {\em const}, the
the compiler attempts to fine an {\tt *IMPLICIT* *CONVER\-SION*} prototype
with a result of the target-type and a single {\tt const} argument, and
if found, replaces $e$ by \\
\centerline{\tt *IMPLICIT* *CONVERSION* ( $e$ )} \\[0.5ex]
using this {\tt *IMPLICIT* *CONVERSION*} function.
