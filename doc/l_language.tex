...
\emkey{area-label}\label{AREA-LABEL} ::=
...
The values of a {\em defined-type-name} are sequences of disjoint
areas.  Each area is a sequence of bits contining possibly
overlapping fields.  If there are no {\tt *AREA*} {\em type-subdeclarations}
in a {\em type-declaration} or its expansions (see below), there is
only a single area named `{\tt main}'.

The {\em type-subdeclarations} are processed in order.  At the
beginning of each, there is a align/pack switch value, a current
area, and an offset-in-bits integer.  These determine the offset in bits
of the next {\em field-declaration} field encountered
relative to the beginning of the current area.
At the beginning of each {\em type-declaration}
the align/pack switch is set to align.  If the {\em type-declaration}
is \underline{not} an expansion (see below),
the current area is set to `{\tt main}'
and the offset is set to zero.  If the {\em type-declaration}
is an expansion, the current area and offset are undefined, and must be
set by an {\tt *ORIGIN*} statement (see below)
before any other {\em type-subdeclarations} can be processed.
...
of an area is the least common multiple of the alignment of
its fields, which, since all alignments are powers of two,
is the same as the largest alignment of any of the fields.
Similarly the alignment of a defined type is the least common multiple
of the alignment of its areas.
...
An `\ttkey{*AREA*} $L$' sub-declaration creates a new area and sets
the current area to the new area and the current origin to zero.
Each area has \underline{only one} {\tt *AREA*} sub-declaration,
and {\em area-labels} are only used in diagnostic messages.  Nevertheless,
the {\em area-labels} of the different areas of a defined type
must be distinct.
...
but current area and offset are undefined.
These must be set by an {\tt *ORIGIN*}
statement (see below) before being used.
Note that a {\em type-declaration} may have several expansions
in different bodies, and the order in which the expansions are compiled
is undetermined, so the `last area and offset' of {\em type-declaration}
is undefined at the beginning of each of its expansions.

The {\em type-declarations} of a defined type form a tree-graph
with the \key{parent} of an expansion being the {\em type-declaration}
with smallest scope containing X.
This tree-graph is called the \key{expansion tree} of the defined type.

A defined type is said to be \key{expandable} at a particular point
in the code if the {\em type-declaration} for the defined type with
smallest scope containing the code point is expandable.  Otherwise
the defined type is said to be non-expandable at the code point.

If a defined type is non-expandable at some code point (equivalently,
if the defined type has a non-expandable {\em type-declaration}),
the expansion tree of the defined type must be linear.  This error
may not be detected until load time.

An defined type cannot be used as a field type if the defined type is
expandable at the point where it is used as the field type.

An defined type cannot be used as the type of a variable allocated
to the function frame if the defined type is
expandable at the point where it is used as the variable type.
However, it can be use as the target type of a pointer allocated with
the `{\tt local}' allocator.

The final size and alignment of a defined type will not be known
until load time if the defined type has expansions.
When these are not known,
values of the type must be
allocated using allocator functions which are passed
the load-time values.  This is done implicitly by calling the
`{\tt local}' allocator for values allocated to the stack,
or may be done explicitly using the `{\tt =@}' operator.
...
A \ttkey{*LABEL*} sub-declaration assigns the current area and offset to the
...
changes the current area and offset to that
...
changes the current offset to the size of the current area,
as it is computed at the point where the {\tt *OFFSET* *SIZE*}
sub-declaration is encountered.  This is just the maximum of all
current offset values computed by previous sub-declarations for
the current area.
