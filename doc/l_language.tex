...
The atomic nature of this write guarentees that all process reads from
and writes to device registers that are in the code before the GO bit
is set are actually done before the GO bit is set.
...
READY bit read guarentees the all process reads from and
and writes to device registers that are in the code after the READY
bit was read as on are actually after the READY bit come on.
\end{indpar}
