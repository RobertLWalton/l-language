...
For variables that are \underline{not} set by an {\em allocation-call} in the
{\em deferred-assignment-statement}, the variables are initialized to zero
and made {\em ro} (not {\em co}) after the {\em deferred-assignment-statement}.
Code that reads these variables before companions compute their values will
...
For pointer variables that \underline{are} set by an {\em allocation-call}
in the {\em deferred-assignment-statement}, the variables are made
{\em co} after the {\em deferred-assignment-statement}, and the pointer
target is given the {\em ro} qualifier, instead of its designated
qualifiers, after the {\em deferred-assignment-statement}.  For most
{\em allocation-calls} the data pointed at will be zero.  Then when
the companion executes, the target will be {\tt *WRITE-ONLY}* in the
{\em statements} of the companion, and after the companion finishes,
will be given the qualifiers of the variable's declarations.
...
The \ttkey{*EXPANDABLE*}\label{*EXPANDABLE*} and
\ttkey{*EXTERNAL*}\label{*EXTERNAL*} sub-declarations allow
a {\em type-definition} to be expanded by a subsequent
{\em type-definition} with the same {\em type-name}.
Only one of these sub-declarations may be given, and it must be the
last sub-declaration in is {\em type-definition}.
The subsequent {\em type-declaration} is called the \key{expansion}
of the {\em type-declaration} that ends with the
{\tt *EXPANDABLE*} or {\tt *EXTERNAL*} sub-declarations.
The expansion must be within the scope\pagnote{SCOPE} of the
{\em type-declaration}.

For a {\em type-declaration} with an {\tt *EXPANDABLE*} sub-declaration,
its expansion must be in the same module\pagnote{MODULE-DECLARATIONS}
as the {\em type-declaration}, or in one of that module's bodies.
For a {\em type-declaration} with an {\tt *EXTERNAL*} sub-declaration
