...
A \ttkey{*EXTERNAL*} sub-declaration sets the external/private switch
...
\hspace*{1in}\{ {\em pointer-type-name} {\em qualifier-name}\STAR{} \}\PLUS{} \\
\hspace*{1.4in}\{ {\em number-type-name} $|$ {\em function-type-name}
			  $|$ {\em defined-type-name} \}
...
{\em pointer-type-name} {\em qualifier-name}\STAR{}
\end{center}
This is a type map (for number, function, pointer, pointer value).
\end{indpar}
...
\ttkey{.final}
\begin{indpar}
The {\tt .final} attribute is {\tt TRUE} if a {\em type-declaration}
for this defined type has been encountered that has no
...
\ttkey{.field type}
\begin{indpar}
The type of the field.  May be a defined type map, function type map,
