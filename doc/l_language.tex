...
the {\em exit-subblock}.  A function declaration is `used' if and only if its
...
argument types.\pagnote{REFERENCE-CALL-PROTOTYPE-MATCHING}
...
whose prototypes match the {\em function-call}.
If there are 0, go to the next step in this algorithm.
If there are more than 1, it is a compile error; terminate this algorithm.
...
whose prototypes match the {\em func\-tion-call}.
...
the prototype matched to the {\em function-call} specifies the
target type of the {\em expression}.  For example, in
`{\tt int z = x + 5}' the target type of {\tt +} is {\tt int},
the prototype matched to `{\tt x +  5}' (the only one with an {\tt int}
...
for a path $d_1,d_2,\ldots,d_n$ from $t$ to {\tt const},
and if found annotate $e$ with $t\mapsto d_1,d_2,\ldots,d_n$,
or if not found,
...
Otherwise search the implicit conversion
graph\pagnote{IMPLICIT-CONVERSION-GRAPH}
for a path $d_1,d_2,\ldots,d_n$ from $t$ to $T$,
and if found annotate $e$ with $t\mapsto d_1,d_2,\ldots,d_n$,
or if not found,
annotate $e$ with $t\mapsto {\tt *FAIL*}$.

\item If $e$ is a {\em function-call}, search for {\em function-declarations}
$d$ whose {\em function-prototype} matches {\em function-call}
$e$ (see Call-Prototype Matching Algorithm below)
and has result type $t$.  When one is found, 
use the prototype to specify the target types of the function arguments, and
apply this algorithm recursively to each argument.  If there is success
for each argument, annotate $e$ with $t\mapsto d$.
It is possible for a second {\em function-declaration}
to also be successful, in which case change the annotation to
$t\mapsto {\tt *AMBIGUOUS*}$.  If no successful {\em function-declarations}
are found, annotate $e$ with $t\mapsto{\tt *FAIL*}$.
...
In order for a {\em function-call} to be matched to a non-special
{\em function-prototype}, the call must have a target type.
If the {\em function-call} is the top level call in a
{\em call-assignment-statement} or {\em expression-assignment-statement},
the {\em result-variable-declarations} of the {\em statement}
provide the target type
(or types if there are several {\em result-variable-declarations}
in a {\em call-assignment-statement}).  Otherwise the {\em function-call}
is an argument to some other function whose prototype specifies the
target type.
...
In searching for a prototype to match the call `{\tt x + ( y * z )}'
with target type {\tt int},
one comes across the prototype `{\tt int r = (int a1) "+" (int a2)}'.
This specifies that the target type of `{\tt y * z}' is {\tt int},
the type of {\tt a2}.  So one then searches for a prototype to
match the call `{\tt y * z}' with target type {\tt int}.

This top-down search can take exponential time unless we memoize the
results.  Therefore for each target type and argument subexpression
we remember one of the following:

\begin{center}
\begin{tabular}{lp{4.0in}}
a {\em function-prototype}
			& if the subexpression is a {\em function-call} that
			  matches the {\em function-prototype}, and only
			  that prototype, given the target type
\\[1ex]
\ttkey{*FAIL*}		& if the subexpression is a {\em function-call} that
                          matches zero or more than one
			  {\em function-prototypes}, given the target type
\\[1ex]
\ttkey{*EXACT*}		& if the subexpression is a
                          {\em reference-expression} or
			  {\tt const} value
			  whose type is exactly the target type
\end{tabular}
\end{center}

If the subexpression is a {\em reference-expression} or {\em const}
...
In addition to the above data memoized for each target type and
argument expression, for each {\em function-call}
when parsed it must be
checked to see if it is a call to a {\tt macro}, {\tt constant}, or
{\tt inclusion} function.  The result of this is the following:

When a {\em function-call} is first parsed, it must be
processed to see if it is a call to a special function.
This processing in turn memoizes the following:

\begin{center}
\begin{tabular}{lp{4.0in}}
a {\em {\tt const}-value}
			& if the subexpression is a {\em function-call} that
			  matches the {\em function-prototype} of a
			  {\tt constant} function whose value is the
			  given {\tt const}-value
\\[1ex]
\ttkey{*NON-SPECIAL*}	& if the subexpression is \underline{not} a call to
			  a {\tt macro} or {\tt constant} function
\end{tabular}
\end{center}

More specifically, when a {\em function-call} is first parsed,
the following algorithm is applied:

\bigskip

\centerline{\bf Function Call Special Algorithm}
\begin{enumerate}
\item Search for \underline{macro} {\em function-declarations}
whose prototypes match the {\em function-call}.
If there are 0, go to the next step in this algorithm.
If there are more than 1, it is a compile error.

Otherwise execute in the compiler the function defined by the
{\em function-declaration}
and replace the {\em function-call} by the result, which must be a {\em const}
value representing a \underline{parsed} {\em expression}. Then if this
parsed {\em expression} is a
{\em function-call}, repeat this step, but if it is something else,
terminate this algorithm.

\item Search for \underline{constant} {\em function-declarations}
whose prototypes match the {\em function-call}.
If there are 0, terminate this algorithm.
If there are more than 1, it is a compile error.
...
Otherwise execute in the compiler the function defined by the
{\em function-declaration},
memoize the {\em const} value returned as the value of the
{\em function-call}, and terminate this algorithm.
...
\centerline{\bf Call-Prototype Matching Algorithm}
\begin{enumerate}
