...
{\em Reference-expressions}\pagnote{REFERENCE-EXPRESSIONS} are
further classified as \key{load} (left of \TT{=}) or \key{store}
(right of \TT{=}).
Load {\em reference-expressions} are evaluated to produce a value.
Store {\em reference-expressions} are evaluated to produce a reference
pointer that is used to store a value.
A {\em reference-expression} with a {\tt const} valued root is evaluated
by this first step.
...
this type is {\tt const} and $E$ is a load {\em reference-expression},
also annotate $E$ with the value $C$ of the variable.  If the type
is {\tt const} and $E$ is a store {\em reference-expression}
annotate $E$ with a reference pointer pointing at the variable
named by $E$.  Then terminate this algorithm.
...
If $E$ is \underline{compound}
{\em reference-expression} whose base has been annotated with a {\tt const}
value,
evaluate the reference pointer of $E$.  If $E$ is a load
{\em reference-expression}, dereference this pointer to get
a {\tt const} value $C$, and annotate $E$ with $C$ and
the type {\tt const}, and terminate this algorithm.
If $E$ is a store {\em reference-expression}, annotate $E$ with
the reference pointer and terminate this algorithm.
...
If $E$ is a load (store) \underline{compound} {\em reference-expression}
(whose base will be annotated with a type by this point),
search for {\em reference-function-declarations} matching $E$.
If a declaration $D$
is found,
