...
A defined type can have several {\em type-declara\-tions}.
All but one of these {\em type-declarations} must be within
the scope of another one of these,
and the set of all {\em type-declarations} of a defined type
forms a tree-graph in which each {\em type-declaration} X is
the root of a subtree containing all {\em type-declarations} Y
that are within the scope\pagnote{SCOPE} of X
(or equivalently, that have a scope
that is a subset of the scope of X).  Thus the \key{parent} of
a non-root Y is the {\em type-declaration} X in the tree-graph of
smallest scope containing Y.

The tree-graph of a defined type is called its \key{expansion tree}.
Non-root {\em type-declarations} in the tree are called
...
the {\tt ***}\label{***} sub-declaration, or all but at most one
...
be external and the expansion tree members can appear anywhere within
...
any branching).  Violation of this rule is detected at load time.
...
can be used as the target of a pointer type.
...
Xa and Xb both beginning with {\tt *OFFSET* *SIZE*}.  If Xb is not
