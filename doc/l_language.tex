...
the UNICODE code point of a {\em character-repre\-sentative}.
The syntax is:
\begin{indpar}
...
A number or character constant may be converted to a run-time
numeric type such as {\tt int32} or {\tt flt64}.
It is a compile error to convert to an integer type that cannot
hold the exact value of the number or character constant.
Conversion
...
A \emkey{rational-constant} is a sequence of
lexemes with specific syntax
that is used to denote a rational number with unbounded
numerators and denominators.  These are stored so that the
...
\begin{indpar}
\emkey{rational-constant}\label{RATIONAL-CONSTANT}
    \begin{tabular}[t]{@{}cl}
    ::= & {\em decimal-constant} \\
    $|$ & {\em binary-constant} \\
...
\\[0.5ex]
\emkey{decimal-constant} ::= {\em decimal-constant-prefix}~
