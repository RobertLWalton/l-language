...
{\em function-declarations} are included in the search.

Furthermore in this step each {\em expression} has a target type (or types),
and {\em function-prototypes} the expression are required to
have results of this type (or results of these types).
...
\pagnote{CALL-ASSIGNMENT-STATEMENTS} with a {\em function-call}
{\em expression} on the right-side, the
...
`{\tt int z = x + 5}' {\tt int} is the target type of {\tt +},
the prototype matched to `{\tt x +  5}' (the only one with an {\tt int}
result type) has {\tt int} result
and argument types, so {\tt int} is the target type of `{\tt 5}'.
...
target types of $e$.  Some of these target types may be the same,
