...
\hspace*{-0.2in}{\tt bool v = x \TT{AND} y} \\
      If {\tt y} evaluates to {\tt FALSE}, same as `{\tt bool v = FALSE}',
      and {\tt x} is not compiled.

      Otherwise same as `{\tt bool v = y if x else FALSE}'.
      Both the {\tt const} values {\tt TRUE}
...
\hspace*{-0.2in}{\tt x \TT{<} y \TT{<} z} \\
      ...
      If two operands of a comparison are both {\tt const}, the comparison
      if reduce to {\tt TRUE} and eliminated, or it is reduced to {tt FALSE}
      which replaces the entire expression.

      In other cases, some operands need to be evaluated
      ...
      works because {\tt int} can be converted to {\tt float},
      but {\tt int} does not work.
