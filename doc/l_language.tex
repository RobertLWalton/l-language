...
\key{(S2)}	& A {\em statement} that is inside a sub-block at the end
                  of a {\em statement} of depth $N$, and is not also
		  inside a sub-block of that sub-block, is given depth $N+1$.
\\[1ex]
...
\\[1ex]
\key{(S4)}	& A {\em statement} in the sub-block at the end of an
                  \underline{out-of-line} function declaration, and also not
		  in any sub-block of that sub-block, is
		  assigned depth 1.
\end{tabular}
\end{indpar}
