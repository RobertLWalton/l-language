...
by {\em include-statements}: see \itemref{INCLUDE-STATEMENTS}.
...
The {\em default-values} are inserted syntactically and evaluate in the
context of the call, and \underline{not} in the context of the
prototype.\footnote{L-Language differs from other compiled languages
in this respect.}
...
Each {\em statement} $S$ has its own \ttkey{*CODE*} {\tt const} variable
...
Before $S$ is compiled its {\tt *CODE*} variable is initialized
to the empty list.  Inline functions compiled during the compilation of $S$
can read and write the {\tt *CODE*} variable of $S$.

Since {\em statements} can be nested, compilation of {\em statements}
can be nested, and at any given time the innermost {\tt *CODE*}
variable hides outer {\tt *CODE*} variables.  Thus at any time
during compilation there is a current {\tt *CODE} variable.
...
\begin{itemize}
\item If the {\em include-argument}
...
Code produced by {\em include-statements} executed during a call to an
inline function
