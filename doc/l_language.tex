...
\subsubsubsection{Deferred and Companion Assignment Statements}
\label{DEFERRED-AND-COMPANION-ASSIGNMENT-STATEMENTS}

The syntax of {\em defer\-red-assignment-state\-ments} and
...
\hspace*{0.5in}
    ...
    \end{tabular}
\\[0.5ex]
{\em block-label} ::= see \pagref{BLOCK-LABEL}
\\[0.5ex]
{\em statement} ::= see \pagref{STATEMENT}
\\[0.5ex]
...
{\em companion-assignment-statements} whose
{\em deferred-vari\-able-declaration}
...
{\em Deferred-variable-declarations} are just {\em block-variable-declarations}
that are not {\em next-declar\-ations}.  A {\em pointer-variable} declared by a
{\em deferred-variable-declaration} may have an {\em allocation-call} in
either its {\em companion-assignent-statement} but not in its
{\em deferred-assignment-statement}.

Code that reads a variable declared in a {\em deferred-assignment-statement}
before the variable's companion executes will read the variable's value
as zero.  For pointers this will usually reference undefined memory which
