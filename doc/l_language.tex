...
default is no lifetime qualifier (a.k.a., undeclared pointer)
which means, roughly, that the pointer is valid only until
...
\ttkey{U} & allows undeclared  & [\ref{Q-FLAG-RULE}] \\
          & \multicolumn{2}{l}{(i.e., neither \TT{*GLOBAL*} ~or \TT{*HEAP*})} \\
...
\key{global} &
	...
	as a {\tt co} variable created by a
...
The first of these rules govern global and heap pointers and variables and are:
\begin{indpar}[0.2in]
...
of a top level {\em statement} has lexical depth 2, and so forth.
...
all the undeclared target variables reachable by tracing
undeclared target variable values starting at $P$ necessarily hold
local variables of depth $D$, or in otherwords, in network of
...
\key{(S4)}	& Let $S$ be the sub-block at the end of an
                  \underline{out-of-line} function declaration that, because
		  it is top level, must be of depth 1.
		  Then any statement in $S$ that is not also
		  in a sub-block of $S$ is assigned depth 2.
\end{tabular}
\end{indpar}
...
containing the inline function currently executing at compile-time.
\end{indpar}
...
in this {\em prototype-argument-declaration} \underline{cannot}
be {\tt const}.
...
\item {\tt *GLOBAL*} may be replaced by undeclared (an anywhere pointer)
\item {\tt *HEAP*} may be replaced by undeclared (an anywhere pointer)
