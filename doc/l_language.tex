...
\subsection{Builtin Arithmetic Operations}
\label{BUILTIN-ARITHMETIC-OPERATIONS}


{\tt N r = \ttkey{"+"} ( N n1 )} \\
{\tt N r = \ttkey{"-"} ( N n1 )} \\
{\tt N r = (N n1) \ttkey{"+"} ( N n2 )} \\
{\tt N r = (N n1) \ttkey{"-"} ( N n2 )} \\
{\tt N r = (N n1) \ttkey{"*"} ( N n2 )} \\
{\tt N r = (N n1) \ttkey{"/"} ( N n2 )}
\begin{indpar}Where {\tt N} is one of the types:
\\[1ex]
\hspace*{1in}\begin{tabular}{llll}
	     \tt flt & \tt flt64 & \tt flt32 \\	
	     \tt int & \tt int64 & \tt int32 \\	
	     \tt uns & \tt uns64 & \tt uns32 \\	
	     \end{tabular}
\\[1ex]
Standard arithmetic operators on numbers {\tt n1} and {\tt n2},
done using IEEE floating point, two's complement, or binary integer arithmetic.

For floating point, the following produce a {\tt NaN} result:
\\[1ex]
\hspace*{1in}\begin{tabular}{l@{\hspace*{1in}}l}
	     \tt +Inf + -Inf & \tt -Inf + +Inf \\
	     \tt +Inf - +Inf & \tt -Inf - -Inf \\
	     \tt +Inf * 0 & \tt 0 * +Inf \\
	     \tt -Inf * 0 & \tt 0 * -Inf \\
	     ...
	     \end{tabular}
\\[1ex]
For floating point, operations may set the following floating point
flags:
\begin{itemlist}
\item[Invalid]
\item[Divide by Zero]
