...
\end{tabular}
\end{center}
After splitting the parts are reclassified; e.g., when {\tt p.@} is split
{\tt p} is a {\em word} but {\tt .@} is a {\em mark}.

Note that {\tt .@} and {\tt .*} are not {\em name-items}, but
{\tt ".@"} and {\tt ".*"} are {\em name-items}.  Function definitions
must use {\em name-items}, but function calls can use the same
lexemes with the {\tt "} quote marks removed.  Thus one can have
a function named {\tt "+"} whose calls use {\tt +} without the quotes.
...
side is a single {\em function-call}.
...
\begin{indpar}[1ex] \tt
reference function QR\$1 T\$1 r =
    {\em ma}\QMARK{} *POINTER* *ACCESS* ( $P$ QR\$1 T\$1 @p ) \\
reference function {\em ma}\QMARK{} *POINTER* *ACCESS* ( $P$ QW\$1 T\$1 @p ) =
    QW\$1 T\$1 r
\end{indpar}

Note that these are reference functions\pagnote{REFERENCE-FUNCTION-DECLARATIONS}
and not non-reference-functions\pagnote{INLINE-FUNCTION-DECLARATIONS}.
A {\tt *POINTER* *ACCESS* *CONVERSION*} function, if it exists,
...
ap int @p:
    p = 5
        // This translates to:
        //   std *POINTER* *ACCESS* ( @p ) = 5
int x = p
  // This translates to:
  //   int x = std *POINTER* *ACCESS* ( @p )
\end{verbatim}\end{indpar}
...
\hspace*{1in}{\em ma1}\QMARK{} *POINTER* *ACCESS* *CONVERSION* ( $P1$ Q\$1 T\$1 @p )
\end{indpar}
which converts a pointer of type $P1$ to a pointer of type $P2$,
where {\em ma1}\QMARK{} denotes the {\em module-abbreviation} of $P1$,
if any.
...
\begin{itemize}
\item
{\em Unparenthesized-actual-arguments} are \underline{not} allowed.
\item
