...
the prototype has an argument named $v$, in which case a call matching
the prototype must match the prototype argument $v$ to a variable
name $w$ for which `\TT{next} $w$' is a legal {\em assignment-result}.
Then `\TT{next} $w$' may be omitted as an {\em assignment-result}
and will be implied.  For example:
...
in the {\tt *INCLUDE*} variable, as described in
...
but set the {\tt *INCLUDE*} variable.
...
each macro function execution adds to or modifies the {\tt *INCLUDE*} variable
...
The function may, however, use the {\tt *INCLUDE*}
