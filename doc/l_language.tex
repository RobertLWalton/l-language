...
In order for a {\em function-call} to be matched to a
...
the {\em result-variable-declarations} provide the target type
...
is an argument to some other function whose prototype specified the
target type.
...
a {\em function-prototype}
			& the subexpression is a {\em function-call} that
			  matches the {\em function-prototype}, and only
			  that prototype, given the target type
\\[1ex]
\ttkey{*FAIL*}		& the subexpression is a {\em function-call} that
                          matches zero or more than one
			  {\em function-prototypes}, given the target type
\\[1ex]
\ttkey{*EXACT*}		& the subexpression is a {\em reference-expression} or
			  {\tt const} value
...
{\tt inclusion} function.  This is referred to as the:

In addition to the above data memoized for each target type and
argument expression, for each argument subexpression (ignoring
target type) we remember one of the following:
...
a {\em {\tt const}-value}
			& the subexpression is a {\em function-call} that
			  ...
			  given {\tt const}-value
\\[1ex]
\ttkey{*NON-SPECIAL*}	& the subexpression is \underline{not} a call to
			  a {\tt macro} or {\tt constant} function
\end{tabular}
\end{center}

The first time we encounter an argument subexpression, we run
the following algorithm and memoize its results:
