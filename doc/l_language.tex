...
\key{(I4)}	& A result of {\tt *LOCAL*} pointer type
		  ...
		  the same depth as the base {\em reference-expression}
...
The rules for assigning depth to arguments and results of
\underline{out-of-line} {\em function-calls} are:
\begin{indpar}[0.2in]
\begin{tabular}{lp{5.0in}}
\key{(O1)}	& An argument of {\tt *LOCAL*} pointer type
                  within an out-of-line {\em function-prototype}
                  invoked by a
                  {\em function-call} is assigned depth 0.
                  the same depth as the corresponding actual call argument.
\\[1ex]
\key{(O2)}	& An out-of-line {\em function-prototype}
		  can\underline{not} have
		  a result of {\tt *LOCAL*} pointer type.
\end{tabular}
\end{indpar}
...
\\[1ex]
		& Note: the pointer variable with {\tt *HEAP*} lifetime
		  will exist as long as the variable (which is {\tt co})
		  exists.
\end{tabular}
\end{indpar}
