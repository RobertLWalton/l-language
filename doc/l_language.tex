...
(from the Bottom-Up Step),
annotate the {\em expression} with $t\mapsto{*EXACT*}$.
Then if $t$ does \underline{not} equal {\tt const},
replace the $e$ with `{\tt *IMPLICIT* *CONVERSION* ( $e$ )}'.

\item If $e$ is a
{\em reference-expression}\pagnote{REFERENCE-EXPRESSIONS} compute its
type $T$ and
annotate the $e$ with $T\mapsto{*EXACT*}$.
Then if $t$ does \underline{not} equal $T$,
replace the $e$ with `{\tt *IMPLICIT* *CONVERSION* ( $e$ )}'.

\item If $e$ is a {\em function-call}, search for a {\em function-declaration}
$d$ whose {\em function-prototype} result type is $t$.  When one is found, 
...
for each argument,
annotate the $e$ with $t\mapsto d$.
