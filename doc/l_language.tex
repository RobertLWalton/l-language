...
The first step in processing a {\em statement}
is to visit in top-to-bottom left-to-right order
all the {\em expressions} in the {\em statement} and apply the following
{\em expression} (macro) replacement algorithm:

\bigskip

\centerline{\bf Top-Down Replacement Algorithm}
\begin{enumerate}
...
If the expression $E$ is a function call,
...
Note that macro processing is done top-down before any other processing on the
{\em statement}.

The second step in processing
a {\em statement} is to visit in bottom-to-top left-to-right order
all the {\em expressions} in the {\em statement} and apply the following
algorithm with evaluates {\em constant-expressions} and analyzes
{\em reference-expressions}:

\bigskip

\centerline{\bf Bottom-Up Constant Evaluation and Reference Expression
                Algorithm}
\begin{enumerate}
...
Note that macro processing precedes this step.

\item\label{REFERENCE-EXPRESSION-STEP}
If $E$ is a {\em reference-expression}, compute the type of its reference
pointer according to the rules in
Reference Expressions, \itemref{REFERENCE-EXPRESSIONS}, and annotate
$E$ with this reference pointer type.  This works if there are no
{\em reference-calls}\pagnote{REFERENCE-CALL} in the
{\em reference-expression}.

For {\em reference-offsets} not involved in {\em reference-calls},
any index {\em expressions} are annotated with the \TT{int}
target type.
...
the {\em function-variable-name}.  If the $E$ is loading, exclude
any {\em store} {\em reference-declaration}, and if $E$ is storing,
exclude any {\em load} {\em reference-declaration}.  If no
{\em reference-declaration} is found (e.g., because $E$ is storing
and there is only a {\tt load} declaration), it is a compile-error.

Otherwise annotate the root of $E$ with $D$ and compute the type of
the reference pointer of the root.  For a {\tt point} $D$
(in which case $E$ must be pointing),
this is the type of the value returned by the function $F$ of $D$.
For {\tt load} or {\tt store} $D$, this is the type of a
(pseudo) virtual pointer\footnote{A virtual reference pointer points
...
a value.}
to the type of the value returned by $F$
(for {\tt load}) or input to $F$ (for {\tt store}).  For {\tt load}
or {\tt store} $D$, the {\em function-variable-name} must be the
\underline{entire} {\em reference-expression} $E$, else there is
a compile-error (because $E$ has no real or virtual reference pointer --
it only has a function to load or store the value of $E$).

The other {\em reference-call} case consists of an already processed
{\em reference-base} with a known reference pointer type
...
compile error.
Note that to get a match, the type of the
...
If $D$ is found with its function $F$,
annotate the {\em reference-call} with $D$ and
with the type of its reference pointer.  
For {\tt load} or {\tt store} $D$, this is the type of a
(pseudo) virtual pointer to the type of the value returned by $F$
(for {\tt load}) or input to $F$ (for {\tt store}).  For {\tt point}
$D$ it is the type of the result of $F$.
Any index {\em expression} in the {\em reference-call} is annotated with 
its target type which is taken from the associated index argument type
in the prototype of $D$ (these index target types are not required
to be {\tt int}).

For all {\em reference-expressions}, whether or not they contain
...
the {\tt *INIT*} qualifier), and for a storing {\em reference-expression},
...
The third step in processing a {\em statement} is to visit in top-down order
...
Top-Down Replacement or Bottom-Up Algorithms).
