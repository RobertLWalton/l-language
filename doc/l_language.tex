...
word size of the computer (typically 32 or 64 bits).
...
An aligned vector pointer {\tt av} is a sequence of 4
integers ({\tt int}) containing:
\begin{itemize}
...
variable name typically begins with exactly $N$ `\TT{@}'s.  However
it is possible that the name begins with fewer that $N$ `\TT{@}'s,
including none at all.  For example:\label{POINTER-DEREFERENCE-EXAMPLE}
\begin{indpar}\begin{verbatim}
...
ap ap *READ-WRITE* ap int @y = ...
    // Now @y is a pointer with 3 cascaded pointer type names.
    // But it can only be dereference once using y.
ap ap *READ-WRITE* ap int z = ...
    // Now z is a pointer with 3 cascaded pointer type names.
    // But it cannot be dereferenced at all.  However,
    // it could be copied to a variable with a different
    // name beginning with 3 @'s, and that variable could be
    // dereferenced up to three times.
\end{verbatim}\end{indpar}
...
max add ( my sum, my increment, z + 100 )
    ...
    //     next my sum += my increment
    //     next my sum = max ( my sum, z + 100 )
...
\ttkey{declaration}
    & The expression must consist of an operator followed by
      an operand (that may contain {\tt =} and {\tt ,}) followed optionally
...
\hspace*{-0.2in}{\tt x \TT{+=} y} \\
	Means `{\tt x \TT{=} x + y}' if {\tt x} is {\tt *READ-WRITE*}
	and `{\tt next x \TT{=} x + y}' if {\tt x} is {\tt co}
	and `{\tt next x}' is defined.
...
\emkey{pointer-variable}\label{POINTER-VARIABLE} ::=
    \begin{tabular}[t]{@{}l@{}}
    {\em variable-name} whose {\em basic-name} may, but need not, \\
    begin with `\TT{@}'s  ~~~~~ [see text below] \\
    \end{tabular} \\
...
\emkey{pointer-member-name}\label{POINTER-MEMBER-NAME} ::=
    \begin{tabular}[t]{@{}l@{}}
    {\em member-name} that may, but need not, have `\TT{@}'s \\
    immediately following the initial `\TT{.}'s \\
    \end{tabular} \\
...
\emkey{data-label}\label{DATA-LABEL} ::=
    {\em basic-name} $|$ {\em member-name} not containing a {\em quoted-mark}
\\[1ex]
...
\item
Pointer types can be cascades: e.g., one can have a pointer to a pointer
to a pointer.  If the cascade has $N$ pointer type names, a
{\em pointer-variable}
with this type typically begins with exactly $N$ `\TT{@}'s.
It may have fewer `\TT{@}'s, including none, but may not have more.
See the example on \pagref{POINTER-DEREFERENCE-EXAMPLE}.

The usual reason for having fewer then $N$ `\TT{@}'s is that
the pointer type contains \TT{PST\$\ldots} and therefore the number of
{\em pointer-type-names} in the pointer type is unknown.

\item
A {\em pointer-variable} beginning with `\TT{@}'s
has associated \emkey{target-variables}
...
is a {\em variable-name} for a non-pointer variable value,
the {\em abbreviating-name} must be a {\em non-pointer-variable},
and if the {\em abbreviated-name} names a variable whose type has
$N>0$ cascaded pointer types,
the {\em abbreviating-name} must be a {\em pointer-variable}
that may begin with up to $N$ `\TT{@}'s, but not more.
...
with the names of the read-only elements.
See \pagref{TYPE-FIELD-SUBFIELD-MAPS}.
...
has two or more {\em pointer-type-names}), set $P$ = the target value of
...
it declares may begin with up to $N$ `\TT{@}'s.
...
{\em call-expression} and the result values of the call,
excluding any results with `\TT{next} $v$'
{\em prototype-next-variable-declarations}%
\pagnote{PROTOTYPE-NEXT-VARIABLE-DECLARATION},
are stored
in the {\em assignment-results} proceeding left-to-right
(see item \ref{CALL-DECLARATION-TARGET-MATCHING},
\pagref{CALL-DECLARATION-TARGET-MATCHING}).
...
The right side of a {\em call-assignment-statement} will have
an implied {\em assignment-result} for every
{\em prototype-next-variable-declaration}%
\pagnote{PROTOTYPE-NEXT-VARIABLE-DECLARATION}%
of the
{\em function-prototype}.  Every such
{\em prototype-next-variable-declaration} `\TT{next} $w$'
must have a corresponding 
a {\em prototype-argument-declaration} `{\TT{\ldots}~$w$}',
and the call {\em actual-argument}
matched to this must be either
a {\tt co} {\em variable} $v$ for which `\TT{next} $v$' is legal,
or a readable and writable {\em reference-expression} $e$
(see item \ref{PROTOTYPE-NEXT-RESULT}, \pagref{PROTOTYPE-NEXT-RESULT},
and also item \ref{CALL-DECLARATION-NEXT-ARGUMENTS},
\pagref{CALL-DECLARATION-NEXT-ARGUMENTS}).
The implied right side result
is then either `\TT{next} $v$' or $e$.  An example is:
\begin{indpar}\begin{verbatim}
...
\emkey{pointer-label}\label{POINTER-LABEL} ::=
    \begin{tabular}[t]{@{}l}
    {\em field-label} beginning with zero or more `\TT{.}'s \\
    followed by zero or more `\TT{@}'s ~~~~~ [see text below] \\
    \end{tabular} \\
\emkey{non-pointer-label}\label{NON-POINTER-LABEL} ::=
    {\em field-label} that has no `\TT{@}'s after any beginning `\TT{.}'s \\
...
it declares typically has exactly $N$ `\TT{@}'s after
any initial `\TT{.}'s.
It may have fewer `\TT{@}'s, including none, but may not have more.
\item
...
offset-in-bits integer (the current offset).
These determine the offset in bits
of the next {\em field-declaration} field encountered
relative to the beginning of each datum of the defined
type being declared.  Also at the beginning of each
{\em type-subdeclaration} is an external/private switch value
that determines whether {\em field-labels} or {\em subfield-labels}
are external (usable by other modules) or private (not usable
by other modules).

There may be several {\em type-declarations}
for the same defined type.
One of these is the \key{root},
or first, of the set of these
{\em type-declarations}, and all the others are called
\skey{expansion}s, and are in the scope of the root.
At the beginning of each {\em type-declaration}
the align/pack switch is set to align, and the external/private
switch is set to private.  At the beginning of the root the
current offset is set to zero, but at the beginning of all others
the current offset is undefined, and must be defined
by an {\tt *OFFSET*} sub-declaration before it is used.
...
If the align/pack switch is in the \key{pack} position and the
next {\em type-subdeclaration} is a {\em field-declaration}, the
current offset will become the offset of the field being declared.

When a field is declared, the current offset is set to the offset of
the field plus the
size of the field, which must be known.  There is also a current
size which is the maximum of all the values of the current offset
computed so far.

If the external/private switch is in the \key{external} position a
{\em field-label} or {\em subfield-label} declared by the
next {\em type-subdeclaration} will be visible throughout the scope
of the {\em type-declaration}'s {\em defined-type-name}, which must
itself be external.  Also the {\em field-type-name} of an external field
{\em type-subdeclaration} must be external.

If the external/private switch is in the \key{private} position a
{\em field-label} or {\em subfield-label} declared by the
next {\em type-subdeclaration} will be only be visible in the
module or body containing the {\em type-subdeclaration}, and if
that is a module, also in the bodies of that module.
...
A \ttkey{*EXTERNAL*}\label{EXTERNAL-SWITCH}
sub-declaration sets the external/private switch
to `external'.  This is allowed only if the {\em defined-type-name}
of the {\em type-declaration} is external
\underline{and}
the current {\em type-declaration} is in the same module as the
root of the {\em defined-type-name}.

A \ttkey{*PRIVATE*} sub-declaration sets the external/private switch
to `private'.

An `\ttkey{*INCLUDE*} {\em defined-type-name}' sub-declaration is similar
to a `{\em defined-type-name} {\em field-label}' sub-declaration except
that instead of defining a new {\em field-label}, all the
fields and subfields of the {\em defined-type-name} are imported to the
defined type containing the sub-declaration.  The final size of the
{\em defined-type-name} must be known (see {\tt ***} below).
...
The root of a defined type with a {\tt *DEFERRED*} {\em type-declaration}
must be within the scope of the {\tt *DEFERRED*} {\em type-declaration}.
If the {\tt *DEFERRED*} {\em type-declaration} is external, the root
must be in the same module as the {\tt *DEFERRED*} {\em type-declaration},
or in one of the bodies of that module.
...
`\ttkey{*OFFSET* *SIZE*}' sets the current offset to the current size.

If a {\em type-declaration} does \underline{not} end with a \ttkey{***}
sub-declaration, expansions of the {\em type-declaration} are forbidden
within the scope of the {\em type-declaration}.  If this {\em type-declaration}
is in the same module or body as the root of its defined type,
then the final size, fields, and subfields of the defined type are fixed
and known within the scope of this {\em type-declaration}.

If a {\em type-declaration} ends with a \ttkey{***}
sub-declaration, expansions of the {\em type-declaration} are permitted
within its scope, and the final size of its {\em defined-type-name} is
\underline{not} known.  Also, if the {\em type-declaration} is
\underline{not} in the same module or body as the type's root,
the final size of the {\em defined-type-name} is \underline{not}
known (it will be computed at load time and used to allocate
values of the defined type).
...
Note that as `{\tt next}' is only recognized as a keyword at the beginning
of a {\em next-variable-declaration}, it can be used as a 
{\em field-label}.
...
it is not an error unless the value is a {\tt const} value (in which
case it is a compile-time error).
...
my type X:
    ...
    X.weight = 152.34
uns8 kind = X.kind
bool animal = X.animal         // == true
bool vegetable = X.vegetable   // == false
flt weight = X.weight
\end{verbatim}\end{indpar}
...
character attributes X:
    ...
    while i < 128:
        ...
        next i = i + 1
bool line feed is graphic = X.is graphic [C#"<LF>"]  // == false
bool A is graphic = X.is graphic [C#"A"]             // == true
\end{verbatim}\end{indpar}
...
allocator function fd *READ-WRITE* file @r = allocate fd:
    data for fd d:
...
\hspace*{0.3in}$D1$ d1 = $D1$ ( @p1 ) \\
\hspace*{0.3in}$D2$ d2 =
    {\em ma}\QMARK{} *POINTER* *IMPLICIT* *CONVERSION* ( d1 ) \\
\hspace*{0.3in}@p2 = *UNCHECKED* ( d2 )
\end{indpar}
...
\hspace*{0.3in}$D1$ d1 = $D1$ ( @p1 ) \\
\hspace*{0.3in}$D2$ d2 =
    {\em ma}\QMARK{} *POINTER* *UNCHECKED* *CONVERSION* ( d1 ) \\
\hspace*{0.3in}@p2 = *UNCHECKED* ( d2 )
\end{indpar}
...
contains $N>0$ {\em pointer-type-names}, then the {\em pointer-\EOL variable}
it declares typically begins with exactly $N$ `\TT{@}'s.
It may have fewer `\TT{@}'s, including none, but may not have more.

If the declaration contains a {\tt PST\$\ldots} wildcard
that is resolved into a list with {\em pointer-type-names},
the {\em prototype-result-declaration} \underline{after}
wildcard substitution will necessarily have fewer
`\TT{@}'s than {\em pointer-type-names}.
\item
...
of the form `\dots{} {\em type-name} $v$'.
\item
Result and argument {\em variable-names}
...
and {\tt Q\ldots\$\ldots} behaves as a {\em qualifier-name}.
\item
...
\begin{indpar}\begin{verbatim}
function F ( int x ) G ( int y ) H ( int z ?= 7 ) I ( int w ):
    . . . . . . . . . .
F ( 10 ) I ( 8 ) G ( 6 )
    // Equivalent to F ( 10 ) G ( 6 ) H ( 7 ) I ( 8 )
\end{verbatim}\end{indpar}
...
However note that the first declaration argument list (here {\tt ( int x )})
may \underline{not} have a default value.
...
or a {\em function-call} to a {\tt constant} function.\footnote{
A {\em rational-constant} is both a {\em constant} and a {\em function-call}
to a {\tt constant} function.}
{\em Constant-expressions} are used to compute parameters that must be
known at compile-time.

Arguments to special functions (such as {\tt constant} functions)
\underline{need not} have the {\tt const}
type.  The values of such arguments cannot be accessed by the functions,
but their types, which are {\tt const} values, can be.
...
continue to the next step with the modified {\em expression}.
...
Note that {\em prototype-next-variable-declarations} are ignored
during this matching.
...
\item\label{CALL-DECLARATION-NEXT-ARGUMENTS}
Some {\em prototype-argument-declarations} may correspond to
{\em prototype-next-variable-de\-clar\-ations} and for these
the {\em actual-arguments} must be either a {\tt co} {\em variable} $v$
for which `\TT{next} $v$' is legal, or a readable and writable
{\em reference-expression}.  However no checks are done on these
requirements during the Function Declaration Matching Algorithm.
These requirements are checked a final call-declaration
match is found, and any requirement failure becomes a compile error.
...
one non-excluded {\em reference-function-declaration},
and it is a compile error
if it has none (e.g., if a storing {\em simple-reference-expression}
...
The type of the {\em reference-base} in the call is matched to the
type of the {\em reference-pattern-base} in the declaration.
If the latter has wildcards, the wildcards are assigned
information from the {\em reference-bases}'s type.
...
inclusion function const r =
    ...
    r = *INCLUDE* ( TYPE-NAME, ALIGNMENT ):
        ...
        allocator function ap QW$1 TYPE-NAME @r =
...
\hspace*{0.5in}{\em out-of-line-function-name}~
	      {\em parenthesized-pattern-argument-list}\PLUS{}
\\[0.5ex]
...
\hspace*{0.25in} \ttkey{function}~
        \{ {\em prototype-result-list}~ \TT{=} \}\QMARK{}~
	\TT{()}~ {\em parenthesized-pattern-argument-list}\PLUS{}
\begin{enumerate}
\item
...
\hspace*{0.25in}
	\ttkey{call}~ {\em function-expression}~
	     {\em parenthesized-call-argument-list}\PLUS{}
\\[0.5ex]
...
\item
The {\em parenthesized-call-argument-list} and the function type's
{\em parenthesized-pattern-argument-list} must match
as if they were part of an out-of-line {\em function-call}
and {\em out-of-line-function-prototype}.
\end{enumerate}
\end{indpar}
...
If a module or body imports another module, it also implicitly
imports\label{IMPLICIT-IMPORTS} all modules imported
(explicitly or implicitly) by that other module.
...
containing the {\em declaration} (a {\em statement} is a
logical line which can end in a {\em block}), and
...
in a body), and must contain a {\em module-abbreviation}.
This {\em module-abbreviation} must name the module containing
the top-level {\em declaration}, except for
companions of external deferred
{\em out-of-line-function-declarations} (see below).
The scope of an external
{\em declaration} is extended to include all modules and bodies that
import, explicitly or implicitly\pagnote{IMPLICIT-IMPORTS},
the module containing the {\em declaration}.
...
{\em module-abbrev\-i\-a\-tion}, except that {\em type-declarations}
or not external if they are expansions of a {\em type-declaration}
appearing in another module.

A body may import two different definitions of a defined type,
one from the module containing the root of the type, and one
from the body's own module which is an expansion of the
first definition.  The unexpanded definition is ignored and the
expanded definition is used.

Components of a {\em type-declaration} (fields, subfields, labels)
are external if they are compiled when their defined type's
external switch\pagnote{EXTERNAL-SWITCH} is on.
The {\em field-type-names} of an external field must be external.
...
However, the prototype argument and result types of an external function
or reference function
must be external.

The companion of a deferred {\em declaration} must be in the same file as the
deferred {\em declaration},
except that if the deferred {\em declaration} is in a module,
its companion may be
in a body of the module (but \underline{not} in modules or bodies that
merely import the module).  If the deferred {\em declaration} is external
and its companion is in a body of its module, files that import the
module are within the scope of the deferred {\em declaration}
but \underline{not} the scope of its companion.
As an exception to the rules just stated,
a deferred external {\em out-of-line-function-declaration}
may have its companion
anywhere within the scope of the original declaration.

A {\em type-declaration} that ends with the {\tt ***}
may have expansions anywhere within the scope of the
{\em type-declaration}.

If two different {\em declarations} of a {\em variable-name},
...
unless one {\em declaration} is deferred and the other is its companion.
It is a compile error if one of these declarations hides another.

The same applies to declarations of {\em defined-type-names},
...
Note that {\em next-variable-declarations} can be implied
...
is rather the context of the inline {\em function-declaration}
...
is rather the context of that {\em function-declaration}.

Similarly when a {\em reference-call} to a reference function is expanded,
the context of the expansion is \underline{not} the current context but
is rather the context of the {\em reference-function-declaration}
that provided the {\em statements} executed by the call.
...
\key{local} &
	...
	or by a {\em prototype-result-variable-declaration}
...
type list element:
    ap list element @next
        // When in the global stack, @next has *GLOBAL*
...
// FIFO implemented as a circular list:
//
ap *GLOBAL* list element @last:  // `@= allocate' is implied
    last.@next = @last
    // Last element is pointed at by last.@next
    // First element is pointed at by last.next.@next
    // Empty list has last.@next == @last
...
ap *READ-WRITE* ap *GLOBAL* list element @@Z:
                        // `@= allocate' is implied.
    // Set @Z to beginning of list and delete it.
    // If list empty, set @Z == @last.
    //
    @Z = last.next.@next    // Also works if last.@next == @last
    if last@next == @last:  // List is empty
        // Do nothing
    else if Z.@next == @Z:  // List has only one element
        last.@next = @last
    else:
        last.next.@next = Z.@next
\end{verbatim}\end{indpar}
...
// Note: @last, @X, @Y, last.@next, X.@next, etc.
// all have depth 1
...
ap *READ-WRITE* ap list element @@Z:
                        // `@= allocate' is implied.
    // Set @Z to beginning of list and delete it.
    // If list empty, set @Z == @last.
    //
...
{\em Out-of-line} undeclared pointer arguments
...
\begin{indpar}\begin{verbatim}
type list element:
    ap *READ-WRITE* list element @next
        // List elements must be *READ-WRITE*

// In the following, `@= allocate' to the local stack is implied
// because the pointers are (lifetime) undeclared.

// FIFO implemented as circular list:
//
...
// Remove an element from the beginning of the circular
// list and return it
//
function ap *READ-WRITE* list element @r =
        remove ( ap *READ-WRITE* list element @last ):
    @r = last.next.@next    // Also works if last.@next == @last
    if last@next == @last:  // List is empty
        // Do nothing
    else if r.@next == @r:  // List has only one element
        last.@next = @last
    else:
        last.next.@next = r.@next

// Take element off list.
//
ap *READ-WRITE* list element @Z = remove ( @last ) 
\end{verbatim}\end{indpar}
...
target of a global pointer is contained in the target of the global
pointer.
...
Space for local variables allocated by {\em variable-declarations}
is allocated to the currently executing out-of-line function
frame or by calls to `{\tt allocate} to the stack after this frame: see
...
\begin{indpar}
These options specify the kind of garbage collection (\key{GC})
...
{\tt separate-stub} option, the pointer to a deallocated body merely points
...
GC runs interleaved with non-GC execution.
The above compiler options control default inline functions that do the
following during non-GC execution:
\begin{itemize}
\item Read a non-pointer from a heap datum.
\item Read a heap pointer from a heap datum.
\item Write a heap pointer to a heap datum.
\item Write a non-pointer to a heap datum.
\item Write a heap pointer to a non-loop iteration stack variable.
\item Write a heap pointer to a loop iteration stack variable.
\item Deallocate a location that contains a heap pointer.
\end{itemize}
...
if {\tt x} is {\tt co} and `{\tt next x}' is defined, or for
`{\tt x = x + y}' if {\tt x} is {\tt *READ-WRITE*}.
...
{\tt constant function const r = std}
    ( const s1 ) \ttkey{"+"} {\tt ( const s2 )}
\begin{indpar}
...
the entry is created and made read-only.
...
{\tt constant function const r = std}
	{\tt ( const m1 )} \ttkey{"+"} {\tt ( const m2 )}
\begin{indpar}
Concatenates the maps {\tt m1} and {\tt m2}.  Specifically,
{\tt r} has the vector elements of {\tt m1} followed by the
vector elements of {\tt m2}, and the dictionary elements
of both {\tt m1} and {\tt m2}.  It is a compile error if
{\tt m1} and {\tt m2} have conflicting dictionary elements,
that is, there is a label such that each map has a dictionary
element with that label but the values of these two elements
are not {\tt ==}.
\end{indpar}
...
Name strings are used in this section to represent type, field, and subfield
names and origin labels.
...
Abbrevations (e.g., {\tt int -{}-{}-{}> std int})
do \underline{not} apply to name strings.  Thus {\tt "int"} is
not used; {\tt "M\$0 int"} is used instead.
...
if {\tt M\$$n$ \rm{maps to} $N$} then both
{\tt com module dictionary[$n$] == "$N$"}
and {\tt com module dictionary["M\$$n$"] == "$N$"}.
For example,
\\[0.5ex]
\hspace*{1.0in}{\tt com module dictionary[0] == "standard"} \\
\hspace*{1.0in}{\tt com module dictionary["M\$0"] == "standard"}

\end{indpar}

\subsubsection{Number Type Maps}
\label{NUMBER-TYPE-MAPS}

At compile-time a {\em number-type-name} can be used as a
{\em variable-name} that names a {\tt co} {\tt const} variable
with a map value called a \key{number type map}.

The compiler defined attributes of a number type map are:
...
\ttkey{.max}
\begin{indpar}
The minimum and maximum values of a number type $N$.
These are {\tt const} values which must be
...
\ttkey{.data type}
\begin{indpar}
The type map of the data type of the pointer type.
\end{indpar}
...
ap = { .kind => "pointer",
       .name => "M$0 ap",
       .data type => M$0 ap data }

M$0 ap data = { .kind => "defined type",
                .name => "M$0 ap data",
                .size => 128,   // Assumes int..size == 64
                .alignment => 64,
                .expandable => FALSE,
                .final => TRUE,
                .components => {
...
\ttkey{.alignment}
\begin{indpar}
Same as for number type maps: see \pagref{SIZE-AND-ALIGNMENT}
\end{indpar}
...
\hspace*{0.2in}\{ {\em pointer-type-name} {\em qualifier-name}\STAR{}
               \}\PLUS{} \\
\hspace*{0.4in}\{ {\em number-type-name} $|$ {\em function-type-name}
			  $|$ {\em defined-type-name}
			  $|$ {\em pointer-type-name} \}

{\tt \ttkey{.kind} => "pointer value"}

\ttkey{.pointer type}
\begin{indpar}
The pointer type map of the first {\em pointer-type-name}
of the pointer value type as.
\end{indpar}
...
{\em pointer-type-name} {\em qualifier-name}\STAR{}
\end{center}
This is a type map (for number, function, pointer,
defined type,
pointer value).
\end{indpar}

\subsubsection{Defined Type Maps}
\label{DEFINED-TYPE-MAPS}

At compile-time a {\em defined-type-name} can be used as a
{\em variable-name} that names a {\tt co} {\tt const} variable
with a map value called a \key{defined type map}.

The compiler defined attributes of a defined type map are:

{\tt \ttkey{.kind} => "defined type"}

\ttkey{.name}
\begin{indpar}
The {\em defined-type-name} of the defined type as a name string.
\end{indpar}
...
defined type definition allows expansions, and is {\tt FALSE}
otherwise.  An expansion not ending in {\tt ***} will set
this {\tt FALSE}.
\end{indpar}

\ttkey{.final}
\begin{indpar}
The {\tt .final} attribute is set {\tt TRUE} by a {\em type-declaration}
for this defined type that has no
ending {\tt ***} and that is in the same module or body as
the root of the defined type.  Otherwise the attribute is
{\tt FALSE}.

This must be {\tt TRUE} if the {\em defined-type-name} is to be
used as a field name or {\tt *INCLUDE*} name.
\end{indpar}

\ttkey{.components}
\begin{indpar}
A dictionary map mapping the {\em data-labels} of the defined type
components (fields, subfields, labels) to their definitions
(field maps, subfield maps, and label maps).

Components may be added during compilation of type expansions.
This map be empty if a type has no components, or if the type is
currently {\tt *DEFERRED*} and not yet defined by the
compilation.

Private components are omitted in the version of this map
that is imported.
\end{indpar}
...
\ttkey{.external}
\begin{indpar}
The {\tt .external} attribute is {\tt TRUE} if the field
is external and {\tt FALSE} if it is private.
\end{indpar}

\ttkey{.parent}
\begin{indpar}
The defined type map of the type of containing this field.
\end{indpar}
...
\ttkey{.field type}
\begin{indpar}
The type of the field.  May be a number type map, function type map,
pointer type map, defined type map, or pointer value type map.
\end{indpar}
...
Subfields are also entries in the parent's {\tt .components}.
\end{indpar}
...
\ttkey{.external}
\begin{indpar}
The {\tt .external} attribute is {\tt TRUE} if the subfield
is external and {\tt FALSE} if it is private.
\end{indpar}
...
Each {\tt *LABEL*} {\em type-subdeclaration} has a {\tt const}
map value called a \key{label map} which is in the {\tt .components}
directory of its parent defined type map.  The compiler attributes of
a label map are:
...
\ttkey{.name}
\begin{indpar}
The name of the label ({\em origin-label})
as a name string.
\end{indpar}

\ttkey{.external}
\begin{indpar}
The {\tt .external} attribute is {\tt TRUE} if the label
is external and {\tt FALSE} if it is private.
\end{indpar}

\ttkey{.parent}
\begin{indpar}
The defined type map of the type of containing this label.
\end{indpar}

\ttkey{.offset}
\begin{indpar}
Offset in bits assigned to this label.
\end{indpar}
...
{\tt constant function com \ttkey{compile error} ( const message )}%
	\label{COMPILE-ERROR} \\
{\tt constant function com \ttkey{compile errorf}
     ( \begin{tabular}[t]{@{}l}
	const format, const a1 = "", \\
	const a2 = "", const a3 = "", \\
	const a4 = "", const a5 = "" )
...
containing the {\tt inclusion} function call
...
the executing macro or inclusion.
\end{indpar}

{\tt constant function const r = com \ttkey{type of} ( const variable name )}
\begin{indpar}
Here {\tt variable name} is the same as in {\tt com name of} and the
result returned is the type of the named variable.
\end{indpar}
...
exactly in a value of an integer type {\tt N}.
For example, converting {\tt 5.5} to an {\tt int} will be a compile
error, while converting {\tt 5.0} to an {\tt int} or {\tt 1e1000}
to a {\tt flt} will not be (in the latter case the result is {\tt +inf}).
...
{\tt allocator function ap QU\$1  PST\$1 r = \\
\hspace*{1in}std ( uns length, uns alignment ) allocate}%
\label{LOCAL} \\
{\tt allocator function av QU\$1  PST\$1 r = \\
...
{\tt allocator function ap QU\$1 *GLOBAL* PST\$1 r = \\
\hspace*{1in}std ( uns length, uns alignment ) allocate}%
\label{GLOBAL} \\
{\tt allocator function av QU\$1 *GLOBAL* PST\$1 r = \\
\hspace*{1in}std ( uns length, uns alignment ) allocate [ uns count ]}
\begin{indpar}
These functions allocate a value of type {\tt PST\$1}
...
uns64 trace file errors = 1 << std new trace ID ( "FILE ERRORS" )
    // Note: 1 is converted to target type uns64 before being shifted.
\end{verbatim}\end{indpar}
...
The registers of an inactive device may be treated as ordinary RAM memory,
as long as only one CPU tries to access these registers.

A device may have a GO bit in one of its registers which if turned on
by a write to that register will activate the device.  It is assumed
that if the value a CPU writes to the register has the same size as the
register, the register write will complete before the device activates.

Similarly a device may have a READY bit in one of its registers which
is turned on when the device deactivates itself.  It is assumed that
if a CPU reads from the register a value of the same size as the register,
and the READY bit is on in that value, then the device has become inactive
before the register read (so the rest of the value read will be that of
an inactive device).
...
location.  The value {\tt r} returned is {\tt true} is {\tt wv} was
written and {\tt false} otherwise.
\end{indpar}
...
device and a process, and has GO and READY bits as described above.
The registers are either owned by the process
and the device is inactive, or the registers are owned by the device
which is actively executing an operation.
...
the GO bit is set by an atomic write of a value whose size matches
the size of the register containing the GO bit.
This activates the device which starts performing the device operation.
...
When the device is active, the register containing the
READY bit is read by an atomic read of a value whose size matches
that of the register containing the READY bit.
The process may do this more than once, until a read reads an on READY bit
...
memory accessible when the device is inactive via two registers: A and D.
A holds the address of
...
in which case A should be written atomically.  If neither A nor D have
atomic types, an {\tt atomic nop} can be put between the write of A and
the operation on D.
\end{indpar}
