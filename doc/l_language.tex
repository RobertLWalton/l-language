...
algorithm which only executes functions that generate \underline{no}
run-time code.  Here {\em expressions} include those discovered using operators
...
first step, but may be used to store {\tt const} values during and after this
first step.  Note that the basis of a compound left
{\em reference-expression} is itself a right {\em reference-expression},
so after the first step left {\em reference-expressions} will either
...
\underline{right} {\em reference-expression}\pagnote{REFERENCE-EXPRESSIONS}
whose base is annotated with a {\tt const} type {\em variable},
