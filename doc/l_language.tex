...
Lifetime depth for some variables is the same as the \key{statement depth}
of the {\em statement} creating the variable.
...
\key{(S2)}	& A {\em statement} that is inside a sub-block at the end
                  of a {\em statement} of depth $N$ is given depth $N+1$.
\\[1ex]
\key{(S3)}	& An \underline{inline} {\em function-call} is treated
                  as creating a sub-block of the {\em statement}
		  containing the {\em function-call}.
\\[1ex]
\key{(S4)}	& {\em Statement} in the sub-block at the end of an
                  \underline{out-of-line} function declaration (and not
		  in any sub-block of that sub-block) are
		  assigned depth 1.
\end{tabular}
\end{indpar}
