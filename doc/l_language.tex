...
\emkey{assignment-call} ::= {\em function-call} $|$
                            {\em foreign-function-call}
\\[0.5ex]
\emkey{assignment-result} ::= see \pagref{ASSIGNMENT-RESULT}
\\[0.5ex]
\emkey{function-call} ::= see \pagref{FUNCTION-CALL}
\\[0.5ex]
\emkey{foreign-function-call} ::= see \pagref{FOREIGN-FUNCTION-CALL}
\end{indpar}
...
Out-of-line function calls must be the right sides (after the {\tt =})
...
\begin{indpar}
\emkey{foreign-function-call}%
	\label{FOREIGN-FUNCTION-CALL} ::= \\
\hspace*{0.25in}
