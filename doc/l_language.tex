...
A non-member names declared by a {\em type-subdeclaration}
are never visible outside {\em type-declarations} in the same expansion tree.
...
\\[1ex]
\key{(L5)}	& A {\tt *LOCAL*} pointer can only be stored in a
                  {\tt *LOCAL*} variable.  It
...
by default must have a pointer type that is the same as the
...
variables whose pointer type has the {\tt *HEAP*} or {\tt *LOCAL*}
qualifier.
\end{indpar}
...
{\tt *LOCAL*} pointers can only be stored in {\tt *LOCAL*} variables.
...
external variables which are given depth 0 (and therefore cannot
store a {\tt *LOCAL*} pointer).
