...
are evaluated to {\tt 4.0} and {\tt 4.5} respectively
before the rules of this section are applied.
The error in converting
{\tt 4.5} to {\tt int} is \underline{not} detected by call-prototype
matching; it is detected when the code produced by call-prototype
matching: \\
\centerline{\tt int y = *IMPLICIT* *CONVERSION* ( 4.5 )}
is compiled.

Note that matching maps are not used in the modified versions
