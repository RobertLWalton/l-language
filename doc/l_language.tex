...
\begin{indpar}[0.2in]
\begin{tabular}{lp{5.0in}}
\key{(I1)}	& An argument within an inline {\em function-prototype}
                  invoked by a
                  {\em function-call} is assigned
                  the same depth as the corresponding actual call argument.
\\[1ex]
\key{(I2)}	& Results of a inline {\em function-prototype} invoked by a
                  {\em function-call} are assigned
                  the same depth as the {\em statement} containing the call.
\\[1ex]
\key{(I3)}	& An argument within a {\em reference-function-prototype}
                  invoked by a
                  {\em re\-ference-call} is assigned
                  the same depth as the corresponding actual call argument.
\\[1ex]
\key{(I4)}	& The result of a {\em reference-function-prototype}
		  invoked by a
