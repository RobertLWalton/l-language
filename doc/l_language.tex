...
the compiler tries to find a function in the current context
with the prototype
\begin{indpar} \tt
function T2 r = {\em ma}\QMARK{} *IMPLICIT* *CONVERSION* ( T1 v )
\end{indpar}
and if the compiler finds such a function,
the compiler changes the code to:
\begin{indpar}\begin{verbatim}
T1 v1 = ...
T2 v2 = *IMPLICIT* *CONVERSION* ( v1 )
\end{verbatim}\end{indpar}
This code will select the function whose prototype was found,
but may select other functions if there is ambiguity.
If there is such ambiguity, the compilation will fail
and the implicit conversion will be unsuccessful.

Note that the requirement that the prototype exist in order
to make the code change prevents implicit conversions from
being cascaded.  That is, if there is implicit conversion
from type {\tt T1} to type {\tt T2}, and implicit conversion
from type {\tt T2} to type {\tt T3}, then implicit conversion
from type {\tt T1} all the way to type {\tt T3} is \underline{not}
implicitly defined.  It may be defined only by
explicitly declaring an {\tt *IMPLICIT* *CONVERSION*} function
with argument of type {\tt T1} and result of type {\tt T3}.
...
the above functions are not used.  Instead the compiler
tries to find a function in the current context
with the prototype
\begin{indpar} \tt
...
\end{indpar}
and if the compiler finds such a function,
the compiler calls {\tt *POINTER* *IMPLICIT* *CON\-VER\-SION*}
to perform the conversion.
This call will select the function whose prototype was found,
but may select other functions if there is ambiguity.
If there is such ambiguity, the compilation will fail
and the implicit conversion will be unsuccessful.
