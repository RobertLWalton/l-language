...
with a ({\em function-}){\em variable-name} possibly preceded by some
...
with a {\em function-declaration} is as follows.  For the corresponding
...
names a pair of functions: one for reading the function variable
and one for writing it.  A {\em function-variable-name} is used
as the root of a {\em reference-expression}.

A reference function can also be invoked by a {\em reference-call}
that invokes a defined by a particular offset, either a {\em member-name}
or \TT{[~]} bracketed expressions.  In this case the reference function
defines the behavior of the offset.
...
\hspace*{1em}\begin{tabular}[t]{rl}
 	  ...
 	  {\em reference-offset} ~
        ...
        & {\em ma} \TT{(} {\em reference-base} \TT{)} ~
 	  {\em reference-offset} ~
          {\em parenthesized-call-term}\STAR{} \\
    \end{tabular}
\\[0.5ex]
{\em reference-offset}\begin{tabular}[t]{rl}
	::= & {\em member-name} {\em parenthesized-call-argument-list}\STAR{} \\
...
\hspace*{0.25in}
    \begin{tabular}[t]{rl}
        ::= & {\em right-reference-function-prototype} \\
        $|$ & {\em left-reference-function-prototype} \\
    \end{tabular}
\\[0.5ex]
\emkey{right-reference-function-prototype}%
    \label{RIGHT-REFERENCE-FUNCTION-PROTOTYPE} \\
\hspace*{0.25in}
    \begin{tabular}[t]{rl}
        ::= & \ttkey{reference function variable} \\
    ...
    \end{tabular}
\\[0.5ex]
\emkey{left-reference-function-prototype}%
\label{LEFT-REFERENCE-FUNCTION-PROTOTYPE} \\
\hspace*{0.25in}
    \begin{tabular}[t]{rl}
        ::= & \ttkey{reference function variable} \\
	    ...
	    & \TT{~~~~~~~~}{\em prototype-result-variable-declaration}
    \end{tabular}
\\[0.5ex]
{\em module-abbreviation} ::= see \pagref{MODULE-ABBREVIATION}
\\[0.5ex]
...
appearing just after the {\em reference-pattern-base}.
\end{itemize}
