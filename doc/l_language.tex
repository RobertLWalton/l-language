...
\begin{indpar}\begin{verbatim}
function macro r = sum ( macro a1, macro a2 ?= { 1 } ):
    r = { a1, "+", a2 };
...
makes the function a \key{constant function}.\label{CONSTANT-FUNCTION}
...
\begin{indpar}\begin{verbatim}
function constant r = sum ( const a1, const a2 ):
    r = a1 + a2;
...
function constant r = list ( const a1, const a2 ):
    r = { a1, a2 }
...
There is really only one difference between a {\tt constant} function
and a non-special function all of whose result types are {\tt const}.
The difference is that when evaluated in the compiler, a
{\tt constant} function is not permitted to compile run-time code,
while a non-special function can compile run-time code.  A consequence
is that a {\em function-call} to a {\tt constant} function can be evaluated
and replaced by the {\tt const} result of the evaluation
without there being any side-effects.
...
that are to be evaluated (as opposed to {\em reference-expressions} on the left
...
If the {\em expression} is a {\em constant}\pagnote{CONSTANTS}, evaluate
...
If the {\em expression} is a
{\em reference-expression}\pagnote{REFERENCE-EXPRESSIONS} whose
root is a {\tt const} type {\em variable},
...
Otherwise annotate the {\em expression} with the first {\tt const} value
returned by the evaluated function
(as in Steps \ref{CONSTANT-STEP} and \ref{CONSTANT-REFERENCE-STEP})
...
{\em call-assignment-statement} by the list of parsed {\em statements}
(see Section~\itemref{INCLUSIONS})
...
\item If $e$ already has an annotation $t\mapsto d_1,d_2,\ldots,d_n$,
do nothing but declare success.
...
or $t'\mapsto d'_1,d'_2,\ldots,d'_n$.
If there is success
...
The first {\em function-term-name} in the call may be followed by
...
if it is, must be preceded by a {\em module-abbreviation} that
...
{\em reference-expression} (e.g., as part of the {\em variable-name}
that is the root of the {\em reference-expression}).
\item
{\em Call-argument-lists} with square brackets are restricted to
appearing just after the {\em reference-call-base}.
\item
...
\\[0.5ex]
{\em input-argument-list} ::= see \pagref{INPUT-ARGUMENT-LIST}
\\[0.5ex]
...
It is surrounded by parentheses {\tt (~)} if and only if it is preceeded by
...
\item[Step \ref{CALL-PROTOTYPE-KEY}%
           \pagnote{CALL-PROTOTYPE-KEY}]
The \key{key} to a {\em reference-function-declaration} is
the type of its {\em reference-pattern-argument} (that is
matched to the {\em reference-call-base}) followed by the
declaration's {\em member-name} or by \TT{[]}.
...
The remainder of the function call that is scanned is
the part after any {\em call-argument-lists} following
either the {\em member-name} or the \TT{[~]} bracketed
{\em call-argument-list} following the {\em reference-call-base}.
...
{\tt function constant r = std (const v1) \ttkey{"=="} ( const v2 )} \\
{\tt function constant r = std (const v1) \ttkey{"!="} ( const v2 )}
\begin{indpar}
...
{\tt function constant r = std \ttkey{type} ( const v1 )}
\begin{indpar}
...
{\tt function constant r = std \ttkey{number} ( const v1 )} \\
{\tt function constant r = std \ttkey{rational} ( const v1 )}
\begin{indpar}
...
{\tt function constant r = std \ttkey{"+"} ( const n1 )} \\
{\tt function constant r = std \ttkey{"-"} ( const n1 )} \\
{\tt function constant r = std (const n1) \ttkey{"+"} ( const n2 )} \\
{\tt function constant r = std (const n1) \ttkey{"-"} ( const n2 )} \\
{\tt function constant r = std (const n1) \ttkey{"*"} ( const n2 )} \\
{\tt function constant r = std (const n1) \ttkey{"/"} ( const n2 )}
\begin{indpar}
...
{\tt function constant r = std (const i1) \ttkey{"\&"} ( const i2 )} \\
{\tt function constant r = std (const i1) \ttkey{"|"} ( const i2 )} \\
{\tt function constant r = std (const i1) \ttkey{"\textasciicircum"}
                            ( const i2 )} \\
{\tt function constant r = std (const i1) \ttkey{"<{}<"} ( const i2 )} \\
{\tt function constant r = std (const i1) \ttkey{">{}>"} ( const i2 )}
\begin{indpar}
...
{\tt function constant r1, const r2 =
...
{\tt function constant r = std} \ttkey{numerator} {\tt ( const r1 )} \\
{\tt function constant r = std} \ttkey{denominator} {\tt ( const r1 )}
\begin{indpar}
...
{\tt function constant r = std} \ttkey{is nan} {\tt ( const v1 )} \\
{\tt function constant r = std} \ttkey{is infinite} {\tt ( const v1 )} \\
{\tt function constant r = std} \ttkey{is finite} {\tt ( const v1 )}
\begin{indpar}
...
{\tt function constant r = std} \ttkey{"\#"} {\tt ( const s )}
\begin{indpar}
...
{\tt function constant r = std} (const s1) \ttkey{"+"} {\tt ( const s2 )}
\begin{indpar}
Returns the concatenation of string {\tt s1}
and string {\tt s2}.
\end{indpar}

{\tt function constant r = std} \ttkey{sprintf}
    {\tt ( \begin{tabular}[t]{@{}l}
...
{\tt function constant r = std (const s1) \ttkey{"=="} ( const s2 )} \\
{\tt function constant r = std (const s1) \ttkey{"!="} ( const s2 )} \\
{\tt function constant r = std (const s1) \ttkey{"<"} ( const s2 )} \\
{\tt function constant r = std (const s1) \ttkey{"<="} ( const s2 )} \\
{\tt function constant r = std (const s1) \ttkey{">"} ( const s2 )} \\
...
{\tt function constant r = std} \ttkey{explode} {\tt ( const s )}
\begin{indpar}
...
{\tt function constant r = std} \ttkey{implode} {\tt ( const m )}
\begin{indpar}
...
{\tt function constant r = std} \ttkey{compile re} ( const s )
\begin{indpar}
...
{\tt function constant r = std} \ttkey{match re} ( const i, const s  )
\begin{indpar}
...
{\tt function constant r = std} \ttkey{scan} ( const s  )
\begin{indpar}
...
{\tt function constant r = std} \ttkey{parse brackets} ( const s  )
\begin{indpar}
...
{\tt function constant r = std} \ttkey{unparse brackets}
...
{\tt function constant r = std} \ttkey{parse} ( const s  )
\begin{indpar}
Equivalent to {\tt \{* {\rm \em value-of-}s *\}}.
...
{\tt function constant r = std} \ttkey{unparse}
	( const v, const f ?= "\%0.16g"  )
\begin{indpar}
...
{\tt function constant r = std} \ttkey{read-write} {\tt ( const m )} \\
{\tt function constant r = std} \ttkey{read-only} {\tt ( const m )}
\begin{indpar}
...
{\tt function constant r = std} \ttkey{read-write} {\tt ( const m, const s )} \\
{\tt function constant r = std} \ttkey{read-only} {\tt ( const m, const s )} %
...
{\tt function constant r = std} \ttkey{"\#"} {\tt ( const m )}
\begin{indpar}
...
{\tt function constant r = std} \ttkey{labels} {\tt ( const m )}
\begin{indpar}
...
{\tt reference function constant r = std}
...
{\tt function constant r = std} \ttkey{copy} {\tt ( const m )} \\
{\tt function constant r = std} \ttkey{copy top} {\tt ( const m )} %
\label{MAP-COPY}
\begin{indpar}
...
{\tt function constant r = std} \ttkey{duplicate} {\tt ( const m )} \\
{\tt function constant r = std} \ttkey{duplicate top} {\tt ( const m )}
\begin{indpar}
Ditto, but if any read-only map is to be copied, the pointer
to the map is copied and no new map is made.
\end{indpar}

{\tt function constant r = std} \ttkey{slice}
...
{\tt function constant r = std} \ttkey{splice}
...
{\tt function constant r = std} \ttkey{truncate} {\tt ( const m, const i )}
\begin{indpar}
...
{\tt function constant r = std} \ttkey{push} {\tt ( const m, const v )}
\begin{indpar}
Appends {\tt v} to the vector of {\tt m} and returns {\tt m}.
\end{indpar}

{\tt function constant r = std} \ttkey{push} {\tt ( const m, const v, const i )}
\begin{indpar}
Executes {\tt push(m,v)} {\tt i} times.  It is an error if {\em i}
is a negative integer.
\end{indpar}

{\tt function constant r = std} \ttkey{append} {\tt ( const m1, const m2 )}
\begin{indpar}
Appends the vector elements of the map {\tt m2} to the vector of {\tt m1}
and returns {\tt m1}.
\end{indpar}

{\tt function constant r = std} \ttkey{pop} {\tt ( const m )}
\begin{indpar}
Deletes the last vector element of {\tt m} and returns it.
Returns {\tt NONE} if {\tt m} is empty.
\end{indpar}

{\tt function constant r = std} \ttkey{pop} {\tt ( const m, const i )}
\begin{indpar}
...
{\tt function constant r = std new exception ID ( const name ) }%
...
{\tt function constant r = std new trace ID ( const name ) }%
\label{NEW-TRACE-ID}
\begin{indpar}
A call to this function pushes {\tt name} to the end of
