...
An {\tt ap} or {\tt fp} pointer part of a reference pointer may point
into the global or local stacks as long as the dereference flag is set.
...
my type Z:       // The Z reference pointer is an `ap my type' pointer
    ...
    Z.X = 5      // The Z.X reference pointer is an `ap int' pointer
                 // to in the stack Z.X with a set dereference flag.
    Z.@Y = null  // The Z.@Y reference pointer is an `ap ap my type'
                 // pointer to Z.@Y in the stack with a set dereference
                 // flag.
