...
is part of the ({\em function-}){\em variable-name}.
...
the reference pointer has already computed.
...
If the declaration {\em reference-prototype-pattern} has a {\em member-name},
the {\em reference-base} must be followed by the same member name,
and this must be followed by either the end of the {\em reference-expression},
or by another {\em reference-offset}, else the call-declaration match fails.

Similarly, if the declaration {\em reference-prototype-pattern}
has a {\em square-bracketed-pattern-argument-list},
the {\em reference-base} must be followed by a
{\em square-bracketed-argument-list} with the same number of elements,
and this must be followed by either the end of the {\em reference-expression},
or by another {\em reference-offset}, else the call-declaration match fails.

\item\label{REFERENCE-CALL-DECLARATION-ARGUMENT-MATCHING}
The {\em reference-base} in the call is matched to the
are matched to the {\em reference-pattern-base} in the declaration.
If the latter has a wildcard, the wildcard is assigned
...
or if different values are assigned to the same wildcard by this
process, the call-declaration match fails.
\end{enumerate}
...
\\[1ex]
If the base {\em reference-expression} in the call is followed by a
{\em square-bracketed-call-argument-list}, the {\em reference-call}
