...
L-Language uses many kinds of names:
...
\emkey{member-name}\label{MEMBER-NAME}
	::= \begin{tabular}[t]{@{}l@{}}
			...
			(also see rule \ref{MEMBER-NAME-RULE} below) \\
			\end{tabular} \\
...
\emkey{function-term-name} ::= {\em name} not containing a `\TT{.}'
	...
	(see rule \ref{FUNCTION-TERM-NAME-RULE} below) \\
\\[1ex]
\emkey{qualifier-name}\label{QUALIFIER-NAME}
    ...
    \end{tabular}
\begin{indpar}
(see rule \ref{QUALIFIER-NAME-RULE} below) \\
...
\begin{enumerate}
\item\label{BASIC-NAME-RULE}
{\em Basic-names} should not begin with a {\em module-abbre\-viation},
contain {\em function-keywords}, begin with {\em initial-operators},
...
\item\label{FUNCTION-TERM-NAME-RULE}
{\em Function-term-names}
should not begin with a {\em module-abbre\-viation}
or contain {\em function-keywords}.  Unless a {\em function-term-name}
is an {\em operator}, it should not begin with an {\em initial-operator},
end with a {\em final-operator}, or contain {\em middle-operators}.
\item\label{MEMBER-NAME-RULE}
{\em Member-names}
should not contain {\em function-keywords}.  Unless a {\em member-name}
is an {\em operator}, it should not
end with a {\em final-operator}, or contain {\em middle-operators}.
\item\label{QUALIFIER-NAME-RULE}
