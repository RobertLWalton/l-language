...
bit was read as on are actually after the READY bit turned on.
\end{indpar}
...
reads or writes D.  If either the write on A or the operation on D
is atomic, it is guarenteed that A will be written before D is read or
written.
\end{indpar}
...
B+1 to B and acquires the lock.  Because the compare-and-set is atomic,
A will be read before the compare-and-set, and all reads an writes
...
will not be corrupted by simultaneous writes by another CPU.
Because the write of B is atomic, another CPU using an atomic read of B
will not see data corrupted by the write of B.
