...
if there is at least one {\em *IMPLICIT* *CONVERSION*} function with
...
\begin{indpar}\begin{verbatim}
P1 ... @p1 = ...
P2 ... @p2 = *POINTER* *IMPLICIT* *CONVERSION* ( @p1 )
\end{verbatim}\end{indpar}
and compiles the rewritten code.  If you define a function
with the prototype
\begin{indpar} \tt
...
successful compile.  Of course such chaining will only work
if there is at least one {\em *POINTER* *IMPLICIT* *CONVERSION*} function with
...
\emkey{pattern-term}\label{PATTERN-TERM}
    ::= {\em non-member-function-term-name}~ {\em pattern-argument-list}\STAR{}
\\[0.5ex]
\emkey{non-member-function-term-name} ::= \\
\hspace*{0.5in}{\em function-term-name} that is not a {\em member-name}
\\[0.5ex]
\emkey{function-term-name} ::= see \pagref{FUNCTION-TERM-NAME}
\\[0.5ex]
...
a function prototype	& the subexpression is a non-{\em const-expression}
                          that is a function call that
...
\ttkey{*EXACT*}		& the subexpression is a reference expression or
			  {\em const-expression}
			  whose type is exactly the target-type or
...
and if the target-type and the type of $e$ are both non-pointer, non-{\tt const}
types, the compiler replaces $e$ by \\
\centerline{\tt *IMPLICIT* *CONVERSION* ( $e$ )} \\[0.5ex]
and attempts to find a function prototype for this revised expression.
If instead the target-type and the type of $e$ are both pointer types,
the compiler replaces $e$ by \\[0.5ex]
\centerline{\tt *POINTER* *IMPLICIT* *CONVERSION* ( $e$ )}

If $e$ is a {\em const-expression} and the target-type
is not {\tt const},
the compiler replaces $e$ by \\[0.5ex]
\centerline{\tt *CONST* *IMPLICIT* *CONVERSION* ( $e$ )}
...
matching: \\
\centerline{\tt int y = *CONST* *IMPLIED* *CONVERSION* ( 5.5 - 5.0 )}
is compiled.
