...
and constants.  Any number or rational constant may be
...
and the only defect of the conversion is a loss of precision
(i.e., a non-infinite number cannot be converted to an infinity).
Any variable may be implicitly converted
at run-time to a type that will hold all the possible values of
the variable, or to any floating point type if the variable type
is a number type (in which case the run-time conversion result
may be an infinity).

In general, L-Language expressions have \skey{target type}s.
For a function call, the function result \underline{cannot}
be implicitly converted to the target type, except that
{\tt const} function results can be implicitly converted
to any target type, unless the conversion violates
the rules of the last paragraph.
...
int z = 1e10		    // illegal; int is target type of 1e10
                            // which is too large to fit
int r1 = 5 + max ( x, y )   // int is target type of 5, max, x, y
int r3 = max ( y, w )       // illegal; int is target type of w
...
const c1 = 100
const c2 = 1000
int r6 = max ( x, c1 + c2 ) // OK; int is target type of const +
\end{verbatim}\end{indpar}
...
\hspace*{0.5in}
    \begin{tabular}[t]{@{}rll}
        & {\em block-variable-declaration}
                \{ \TT{,} {\em block-variable-declaration} \}\QMARK{} \TT{=} \\
...
The {\em expressions} are evaluated in left to right order, and
