...
value in memory a lifetime type and a separate lifetime depth.
...
to functions.  A {\em statement} that is not inside any
sub-block of another {\em statement} that is \underline{not} a declaration
of a function is assigned depth 1.
A {\em statement} that is inside a sub-block
at the end of a {\em statement} of depth $N$ is given depth $N+1$.
Inside a function, statements that are outside the function are given
depth 0.

Then the \skey{lifetime rule}s for storing pointers in values are:

\begin{indpar}
(L1) If a pointer to a value $v1$ of lifetime depth $d1$ is stored
in a value $v2$ of
lifetime depth $d2$, it is required that $0<d1\leq d2$ or that the
lifetime type of $v1$ be {\tt *GLOBAL*}.
\end{indpar}
