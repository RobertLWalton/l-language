...
must be a {\em reference-expression} that is not a {\tt constant}
and does not have the form `{\tt next} {\em variable-name}'.
\item
Result and argument {\em variable-names}
...
\begin{indpar}\begin{verbatim}
function macro r = sum ( macro a1, macro a2 ):
    r = { a1, "+", a2 };
...
flt64 r1 = x + 5
    ...
    // flt64 <--- const exist.
bool b1 = ( x == 5 )
    // Target type is bool.  Type int32 of reference
    // expression x works because implicit conversion
    // int32 <--- const exists.
flt64 y = 5.5
bool b2 = ( x == y )
    // Target type bool.  Reference expression y
    // selects N = flt64 for "==" which works because
    // implicit conversion flt64 <--- int32 exists.
    // Reference expression x selects N = int32 for "=="
    // which fails because implicit conversion
    // int32 <--- flt64 does NOT exist.
bool b2 = ( flt64 ( x ) == 5 )
    // The explicit type conversion function flt64
    // selects N = flt64 which works because
    // flt64 <--- const exists.
\end{verbatim}\end{indpar}
