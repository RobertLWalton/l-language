...
\emkey{block-control-statement}\label{BLOCK-CONTROL-STATEMENT}
	::= {\em goto-exit-statement} | {\em throw-exception-statement} \\
{\em go-to-exit-statement} ::= see \pagref{GO-TO-STATEMENT} \\
{\em throw-exception-statement} ::= see \pagref{THROW-EXCEPTION-STATEMENT}
\\[2.0ex]
where
\begin{itemize}
...
executes a:
\begin{indpar}
\emkey{throw-exception-statement}\label{THROW-EXCEPTION-STATEMENT} ::=
    \ttkey{throw} \TT{*EXCEPTION*}
\end{indpar}
It is also entered if any preceeding statement execution has a fault,
such as a memory fault or an integer divide-by-zero fault.
If a {\tt throw *EXCEPTION*} statement is executed in an
{\em exception-subblock}, it behaves as if the containing
{\em block-assignment-statement}
were replaced by a {\tt throw *EXCEPTION*} statement.  If a
...
statement was a {\tt throw *EXCEPTION*} statement.
...
{\tt const std exception ID map = \\
\hspace*{0.3in}\{ "NONE", "DIVIDE", "INACCESSIBLE", "WRITE" \} } \\
{\tt const std trace ID map = \{ "NONE" \}}
...
This function should only be used inside {\tt *EXCEPTION*}
{\em exit-subblocks}\pagnote{EXCEPTION-SUBBLOCK}.
\end{indpar}

\subsection{Builtin Exeption and Trace Support Operations}
\label{BUILTIN-EXCEPTION-AND-TRACE-SUPPORT-OPERATIONS}

TBD

An L-Language \key{ID} is an {\tt uns} that identifies something.
{\em Throw-exception-statements} may have \skey{exception ID}s
and {\em trace-statements} may have \skey{trace ID}s.

\begin{center}
\begin{tabular}{lp{3.0in}}
\TT{std divide exception} & Integer divide by zero. \\
\TT{std inaccessible exception} & Read or write of inaccessible
                                  memory location.  \\
\TT{std write exception} & Write of accessible but unwritable
                           memory location.  \\
