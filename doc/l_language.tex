...
is a {\em type-name}, the {\em function-declaration} must have
a {\em prototype-result} with exactly
one {\em prototype-result-variable-declaration}
that has for its type this same {\em type-name},
...
\label{EXPLICIT-TYPE-CONVERSION-FUNCTION}, and behaves like a
{\em reference-expression} in that its type is known when it
is encountered by a top-down traverse of {\em expressions}.
\item
...
one of the operands must be a {\em reference-expression} or a
{\em function-call} to an explicit type conversion function whose type can be
...
\item If $e$ is a
{\em reference-expression}\pagnote{REFERENCE-EXPRESSIONS} or
a {\em function-call} to an
explicit type conversion func\-tion\pagnote{EXPLICIT-TYPE-CONVERSION-FUNCTION},
compute its type $T$. 
