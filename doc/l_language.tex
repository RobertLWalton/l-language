...
and ignores actual argument types that match macro prototype arguments,
...
During execution, a macro function cannot access non-macro
...
A {\em macro-statement} is a statement of the form:
\begin{indpar}
...
While the macro function is executing
the entire parsed {\em macro-statement} is the value of the
...
The macro function may alternatively return an {\em expression}
...
Macro functions are commonly used to translate user defined declarations
...
where only prototypes all of whose arguments
and results have the {\tt const} type are considered.
\end{enumerate}

When a {\em const-expression} is found by the bottom-up
transversal, it is immediately evaluated at compile time,
and replaced by its {\tt const} value, unless it is the
{\em function-call} in a
{\em call-assignment-statement}\pagnote{CALL-ASSIGNMENT-STATEMENTS}.
In this latter case, the evaluted {\em function-call} instead sets
the {\em call-assignment-statement's} {\em assignment-results}.
...
will be compiled immeidately after evaluation of the
the function that produced the code.
