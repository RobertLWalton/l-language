...
types are ignored).
They use the reference pointer and index values as arguments.
\end{itemize}
...
\hspace*{0.5in}\begin{tabular}{l@{\hspace*{1in}}l}
	     ...
	     \end{tabular}
\\[1ex]
Also set when an operand is a signalling NaN, in which case the
...
coes not change the exception register.
...
Note that a single atomic operation may execute both reads and writes.
...
done by the current process that would be \underline{after}
this atomic operation in strict sequential program execution
are done after this atomic operation.
\item
All operations on memory (reads, writes, atomic or not-atomic)
done by the current process that would be \underline{before}
this atomic operation in strict sequential program execution
are done before this atomic operation.
\item
This atomic operation's action on RAM
cannot be interrupted by another atomic operation's action on RAM,
where the other atomic operation is executed by a different CPU
or process,
if this operation reads a location
and the other operation writes the location, or if this operation writes
a location and the other operation reads the location.

Note that this applies only to RAM memory, and not to input/ouput
device memory.
\end{enumerate}
\end{indpar}
...
pointed at by {\tt p} and returns the value read.
\end{indpar}
...
The GO bit is set by an atomic write, which guarentees that all reads from
and writes to registers in the code before the GO bit is set are actually done
before the GO bit is set.

The READY bit is read by an atomic read, which guarentees that all reads from
and writes to registers in the code after the READY bit has been
read as being on
...
However, we assume that during the interval beginning with the write of A
and ending with the read of D, no other process reads or writes the registers
or memory of the device, and the device itself does not read or write
its registers or memory.
\end{indpar}
...
an atomic compare-and-set on B that checks that B == A and if so writes
B+1 to B and acquires the lock.  The atomic rules guarantee that
the read of B is done after the read of A, and all reads and writes
of the data structure are done after the compare-and-set.
...
The atomic rules guarantee that all reads of the data structure
by the current process
are done after the read of A and before the read of B.
