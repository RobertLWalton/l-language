...
\item A \key{call part}.  May be (usually is) missing.  If present
this is the declaration of a reference function plus values of any
indices used by that function.  This function must be called with
the pointer part and indices to 
...
The type of a reference pointer is defined to be the type of the value
...
We begin with the root, which has the following cases:
\begin{enumerate}
...
may \underline{not} be missing), dereference flag $D$, and call part $C$ (which
may be missing).  Let the reference pointer that is the result of
the operation have pointer part $P'$,
dereference flag $D'$, can call part $C'$.  Then the computation has
the following cases:\label{FIELD-SELECTION}
\begin{enumerate}
...
If the pointer type of $P$ is \underline{not} builtin
...
If found, set $P=P${\tt ..free} and go back to the beginning
of this algorithm (check Case 1 again).

Note that Case~\ref{OFFSET-CASE} has precedence over this case.

\item\label{POINTER-CASE}
If the target of $P$ is itself a pointer (the type of $P$
has two or more {\em pointer-names}), set $P$ = the target value of $P$ and
$D$ = clear, and go back to the beginning
of this algorithm (check Case 1 again).
...
\item\label{DP-CASE}
If $P$ is a {\tt dp} pointer pointing at a user defined type and the
...
If $P$ is an {\tt ap} pointer pointing at a user defined type and the
...
If $P$ is an {\tt av} or {\tt fv} pointer and the
...
the qualifiers of the resulting
...
always set.  The call part may or may not exist.  There are no
{\tt point} compile-time reference functions.  All indices must
be {\em constant-expressions}, and the reference pointer of a
{\em reference-expression} is completely computed at compile-time.

The root has the following cases:
\begin{enumerate}
\item If the root is a {\em variable-name} or
`\TT{next} {\em variable-name}', then the pointer part of the root's
reference pointer points into the compile-time local or global stack
and the call part does not exist.
\item If the root is the {\em function-variable-name} of a
{\tt store} reference function and the root is storing
(so the root must be the \underline{entire} {\em reference-expression}),
then the pointer part of the root's
...
and the root is loading,
then the pointer part of the root's
reference pointer does not exist and the call part consists of the
declaration of the {\tt load} reference function without any indices.
\end{enumerate}
