...
It is not necessary to include the {\tt .position} dictionary
entry in {\tt *CODE*} statements or expressions.  Nor it is
necessary to include the following {\tt .initiator}s or
{\tt .terminator}s:
...
because logical lines can be identified from context and
{\tt ()} bracketted subexpressions are equivalent to
subexpressions with implied brackets.

However, the following must be included appropriately:
...
A {\tt .separator} token must also be included when indicated.
