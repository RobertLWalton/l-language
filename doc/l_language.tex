...
one of the operands must be a {\em reference-expression}
whose type can be used as the target type of all the operands
(in this case, the type of `{\tt x}' becomes the target type of `{\tt y + 5}'),
or one of the operands is
an explicit type conversion\pagnote{EXPLICIT-TYPE-CONVERSION}
whose type becomes the target type of all the operands
(as in `{\tt x < flt ( y + 5 )}').
