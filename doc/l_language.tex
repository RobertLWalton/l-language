...
a \TT{[} or {\em member-name}.  The base may be parenthesized, and
if parenthesized, may be
preceded by a {\em module-abbreviation} that is part of a
{\em reference-call}.

So a {\em reference-expression} begins with a {\em variable-name} or
`\TT{next} {\em variable-name}' that may only
be preceded
by explicit or implicit left parentheses and {\em module-abbreviations}.
...
may be implied.
\item
An {\em expression} with no operators that begins with a {\em variable-name}
or `\TT{next} {\em vari\-able-name}'
or an explicitly or implicitly parenthesized {\em expression}
followed by a `\TT{[}' or a {\em member-name} and
possibly preceded by a {\em module-abbreviation}
is a {\em reference-expression}.

Otherwise an {\em expression} with no operators is a
(non-reference) {\em function-call}\pagnote{FUNCTION-CALL}
or {\em constant}\pagnote{CONSTANT},
unless it consists of just
an explicitly or implicitly parenthesized subexpression optionally
preceded by a {\em module-abbreviation}
(see \pagref{MA-SYNTACTIC-SUGAR} for more information about this case).
\end{itemize}
\end{indpar}
...
\item {\em Call-argument-lists} are explicitly or implicitly
\TT{(~)} parenthesized,
as opposed to {\em reference-call-argument-lists}\pagnote{REFERENCE-CALL-ARGUMENT-LIST} that are \TT{[~]} square bracketed.
\end{itemize}
\end{indpar}
...
Also \TT{[~]} bracketed lists cannot be {\em call-argument-lists},
a syntactic distinction to their
...
{\em pattern-argument-lists} before the first
...
algorithm.  Here {\em expressions} include those discovered using operators
and {\em reference-expressions}, including those that are bases for
larger {\em reference-expressions} (e.g., `{\tt x[3,4].weight}' has
bases `{\tt x}' and `{\tt x[3,4]}').  {\em Reference-expressions} are
further divided into \key{left} (of \TT{=}) and \key{right} (of \TT{=}).
Right {\em reference-expressions} are evaluated to produce a value.
Left {\em reference-expressions} are evaluated to produce a storage location
into which a value may be stored.  Note that the basis of a compound left
{\em reference-expression} is itself a right {\em reference-expression}.
As the first step concerns only {\tt const} values, the only locations
are variables in the compiler function execution stack and map-index
pairs.
...
If $E$ is a \underline{compound}
{\em reference-expression}\pagnote{REFERENCE-EXPRESSIONS} whose
base is annotated with a {\tt const} value, search for {\tt macro}
...
If $E$ is a (simple or compound)
{\em reference-expression}\pagnote{REFERENCE-EXPRESSIONS} whose
base is annotated with a {\tt const} type {\em variable},
evaluate $E$ to a {\tt const} value $C$, annotate $E$
with $C$, and terminate this algorithm.
...
(4) a bracketed argument list (with {\tt (~)} or
implicit {\tt (~)} brackets).
...
\hspace*{1em}\begin{tabular}[t]{rl}
	  ...
	  \end{tabular} \\
        & {\em reference-call-base} ~
	  \begin{tabular}[t]{@{}l@{}}
	  {\em reference-call-argument-list}\PLUS{} \\
          {\em call-term}\STAR{} \\
	  \end{tabular} \\
    \end{tabular}
\\[0.5ex]
...
\\[0.5ex]
\emkey{call-argument-list}\label{REFERENCE-CALL-ARGUMENT-LIST}
    \begin{tabular}[t]{rl}
    ::= & \TT{[} {\em actual-argument}
          \{ \TT{,} {\em actual-argument} \}\STAR{} \TT{]} \\
    $|$ & \TT{[~]} \\
...
\item
{\em Reference-all-argument-lists} with square brackets are restricted to
...
\hspace*{0.25in}
    \begin{tabular}[t]{rl}
        ::= & {\em function-specializer} \ttkey{reference function} \\
    ...
    \end{tabular}
\\[0.5ex]
{\em function-specializer} ::= see \pagref{FUNCTION-SPECIALIZER}
\\[0.5ex]
{\em module-abbreviation} ::= see \pagref{MODULE-ABBREVIATION}
\\[0.5ex]
{\em prototype-result-variable-declaration} ::=
...
\hspace*{2em}\begin{tabular}[t]{rl}
    ...
    $|$ & {\em reference-pattern-argument}~
          \begin{tabular}[t]{l}
          {\em reference-pattern-argument-list}\PLUS{} \\
	  \end{tabular} \\
    \end{tabular}
\\[0.5ex]
{\em member-name} ::= see \pagref{MEMBER-NAME}
...
\\[0.5ex]
\emkey{reference=pattern-argument-list}%
    \label{REFERENCE-PATTERN-ARGUMENT-LIST} \\
\hspace*{0.5in}
    \begin{tabular}[t]{@{}rl}
    ::= & \TT{[} {\em prototype-argument-declaration}~
	     \{ \TT{,} {\em prototype-argument-declaration} \}\STAR{} \TT{]} \\
