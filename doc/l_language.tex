...
the GC updates all pointers in the stack, and this does \underline{not}
...
Inline functions that that {\tt const} results and arguments
execute at compile-time.  The functions described in the
following sections are builtin compile-time functions.
Unless stated otherwise, they obey the following rules:
\begin{enumerate}
\item
Boolean values are represented by the special values
{\tt TRUE} and {\tt FALSE}.
\item
...
the {\tt UNDEF} special value and producing
...
{\tt const r =} \ttkey{numerator} {\tt ( const rational )} \\
{\tt const r =} \ttkey{denominator} {\tt ( const rational )}
\begin{indpar}
...
At compile-time a type-name is a {\tt const} map value
called a \key{type map}.
A type map is read-write as a whole, but some of its
labels are protected.  The following are protected labels
provided by the compiler.  Unless otherwise specified,
these have values that do not change during compilation.

Compiled code may add its own labels to a type map.
To prevent conflict, the labels provided by the compiler
begin with `{\tt .}', so
that to use them you must use double dots: `{\tt ..}'.
E.g. {\tt int..size}.
...
May increase during compilation of {\tt *EXPANDABLE*} or
{\tt *EXTERNAL*} types.
\end{indpar}
...
type is {\tt *EXPANDABLE*} \underline{or} {\tt *EXTERNAL*},
...
a type map.  The compiler defined labels of a field map are:
