...
\\[0.5ex]
\emkey{dictionary-label}\label{MAP-LABEL} ::= {\em data-label}
...
entries are read-only.  This can be changed: see \pagref{READ-ONLY-FLAG}.
...
the list of statements to be compiled once the statement
containing the function call is compiled.  It is an error
if there is more than one function call doing this in
the statement containing the function call that sets the
{\tt *CODE*} variable.

It is not necessary to include some dictionary
entries in {\tt *CODE*} statements or expressions.

Specifically, it is \underline{not} necessary to
include the {\tt .position} entry or
the following {\tt .initiator}s or {\tt .terminator}s:

\begin{center} \tt
{\rm \bf Unnecessary Dictionary Entries}
\\[1ex]
\begin{tabular}{l@{~~~~~~~~~~}l}
...
subexpressions with implied brackets.

However, the {\tt .separator} token and the following must be included:

\begin{center} \tt
{\rm \bf Necessary Dictionary Entries}
\\[1ex]
\begin{tabular}{l@{~~~~~~~~~~}l}
.initiator & .terminator
\\[1ex]
...
instance begin substituted is an element of a list,
the {\em include-argument} list is spliced into the instance containing
list.
Thus if {\tt X = \{"A", "B"\}} then the {\em statement} {\tt Y = X 1} becomes
{\tt Y = A B 1}.  The {\em include-argument} value list may be empty.

\item Otherwise the value of the {\em include-argument} replaces the
instance in the parsed {\em statement}.
Thus if {\tt X = \{ "A" "+" "B"\}} then the {\em statement}
{\tt Y = X * 2} becomes the equivalent of
{\tt Y = ( A + B ) * 2} where the parentheses are implied.
\end{itemize}
...
A map may be read-write or read-only.  Read-only maps cannot
