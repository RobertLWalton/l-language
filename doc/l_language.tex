...
The algorithm for finding a {\em function-declaration} whose prototype
matches a {\em function-call} is as follows:

\centerline{\bf Function Prototype Search Algorithm}
\begin{enumerate}
...
Otherwise execute in the compiler the function associated with the prototype
and replace the {\em function-call} by the result, which must be a {\em const}
value representing a \underline{parsed} {\em expression}. Then process this
{\em expression}: e.g., if it is also a {\em function-call}, repeat this
step for that {\em function-call}.
...
Otherwise execute in the compiler the function associated with the prototype
and replace the {\em function-call} by the result which must be a {\em const}
value.  Then process this
result: e.g., if the {\em function-call} is at the
top level of a {\em call-assignment-statement}, store the {\em const}
value in the first result variable.
of the {\em call-assignment-statement}.

In general, the situation is the same as if the word
{\tt constant} in the prototype were replaced by {\tt const}
and Step \ref{UNLABELED-MATCH} were executed.  What the word {\tt constant}
does is promise that the inline function will not produce any run-time code
when it is executed at compile-time.

\item If the {\em function-call}
is \underline{not} at the top level of a {\em call-assignment-statement},
go to the next step in this algorithm.
Otherwise search for \underline{inclusion} {\em function-declarations}
whose prototypes match the {\em function-call}.
If there are 0, go to the next step in this algorithm.
If there are more than 1, it is a compile error.

Otherwise execute in the compiler the function associated with the prototype
and replace the {\em call-assignment-statement} by the result which must
be a {\em const} value representing a sequence of \underline{parsed}
{\em statements}.
Then process these {\em statements}.

\item\label{UNLABELED-MATCH}
Search for {\em function-declarations} that are \underline{not}
{\tt macro}, {\tt constant}, or {\tt inclusion} declarations and
whose prototypes match the {\em function-call}.
If there are 0 or 1, it is a compile error.
Otherwise execute in the compiler the function associated with the prototype
to optinally produce run-time code.
\end{enumerate}
