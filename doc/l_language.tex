...
to form the address of the vector element
...
shorter than an {\tt int} integer, and therefore the
...
{\tt int} while `{\tt @Y.op code}' is a {\tt co} pointer to
a {\tt *READ-WRITE*} {\tt int}.
...
with {\tt co} integer elements during compilation.
...
the pointer conversion may be illegal, as will be detected
when this function is called with these types.
...
\\\hline
\ttkey{BUT NOT} & logical and not & infix & binary & 3000
\\\hline
\ttkey{AND} & logical and & infix & n-ary & 3100
\\\cline{1-2}
\ttkey{OR}  & logical or  & & &
\\\hline
\ttkey{NOT}  & logical not & prefix & unary & 3200
\\\hline
\ttkey{==}  & equal & infix & left & 4000
\\\cline{1-2}
\ttkey{!=}  & not equal & & associative &
\\\cline{1-2}
\ttkey{<}  & less than & & &
\\\cline{1-2}
...
\ttkey{\&}  & bitwise and & & &
\\\cline{1-2}
\ttkey{\textasciitilde}  & bitwise xor & & &
\\\hline
\ttkey{/}  & division & infix & binary & 5100
\\\cline{1-2}\cline{4-5}
\ttkey{*}  & multiplication & & n-ary & 5200
\\\cline{1-2}\cline{4-5}
\ttkey{\textasciicircum}  & exponentiation & & binary & 5300
\\\hline
...
\ttkey{left}	& {\em P-operator} must be immediately
                  preceeded by a {\em (P+1)-expression}
                  in its {\em P-expression}. \\
...
\ttkey{afix}	& {\em P-operator} must be after a (not necessarily
                  immediately) preceeding {\em P-operator}
                  in its {\em P-expression}. \\
\end{tabular}
\end{center}
...
All of these but {\tt inital} and {\tt prefix} can be combined with {\tt afix}.
...
\\[0.5ex]
\begin{tabular}{p{1in}p{5.0in}}
\ttkey{\begin{tabular}[t]{@{}l}left\\associative\end{tabular}}
    & The expression must consist of alternating operands
      and operators and begin and end with an operand.
      As long as the expression contains at least two operators,
      the begining subsequence `operand operator operand'
      is enclosed in \TT{\{|~~|\}} brackets to make it into
      a single operand (e.g., `{\tt X == Y < Z}' becomes
      `{\tt \{| X == Y |\} < Z}'.
\end{tabular}
\\[0.5ex]
\begin{tabular}{p{1in}p{5.0in}}
...
\tt x \TT{if} y & evaluates {\tt y}, and if the value of that is {\tt true},
                  evaluates and returns the value of {\tt x}; otherwise
		  returns {\tt NONE}
\\[0.5ex]
		& NOTE: {\tt NONE} cannot be stored in a variable, but
		  can be converted to {\tt false} which can be stored
		  in a {\tt bool} variable.
\\[0.5ex]
\tt x \TT{else} y & evaluates {\tt x}, if the value of that is {\tt NONE},
                  evaluates and returns {\tt y}; otherwise the value of
		  {\tt x} is returned without evaluating {\tt y}
\\[0.5ex]
\tt x \TT{<} y    & returns either {\tt NONE} or {\tt \{?~value-of-y~?\}};
		    evaluates {\tt x} and if that value is {\tt NONE},
		    returns {\tt NONE}; otherwise evaluates {\tt y},
		    and if the value of {\tt x < y} is {\tt false},
		    returns {\tt NONE}, but if the value of {\tt x < y}
		    is {\tt true}, returns {\tt \{?~value-of-y~?\}}.
\\[0.5ex]
		& NOTE: {\tt \{?~x~?\}} is converted to {\tt x}
		  if it is the value of the first operand to a
		  comparison operator, and can be converted to {\tt true}
		  in other contexts if a {\tt bool} value is required.
\\[0.5ex]
...
\hspace*{0.5in}\TT{\{}
    ...
    \end{tabular}
\\[0.2ex]
where {\em quoted-name-item} is a {\em name-item} with {\em words} turned into
{\em quoted-strings}.
...
of a {\em map}.  An {\em index} can also be a {\em const}
...
the string must be used to select a {\em map-dictionary}
...
If the {\em variable-name} at the beginning of a {\em reference-expression}
is a {\em target-variable} with an assocated {\em pointer-variable},
...
its assocated {\em pointer-variable}.

If the {\em variable-name} at the beginning of a {\em reference-expression}
is a {\em target-variable} \underline{without}
a assocated {\em pointer-variable},
the reference expression refers to variable, field, or vector element
allocated to the current function frame.
...
by the syntax `{\em map-label} {\tt =>} {\em constant-expression}'.
