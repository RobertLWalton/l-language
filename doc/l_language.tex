...
{\tt function const r = std} \ttkey{unparse brackets} ( const v  )
\begin{indpar}
Inverse of {\tt parse brackets}.  Return a string $S$ such
that {\tt `$S$'} equals {\tt v}.  Note that numbers may be very
slightly changed due to rounding.
\end{indpar}
...
{\tt function const r = std} \ttkey{unparse} ( const v  )
\begin{indpar}
...
{\em variable-name} that names a map value
