...
Recall that {\tt macro}, {\tt constant}, and {\tt inclusion}
functions are \key{special functions}.  The first step in processing
a {\em statement} is to visit all the {\em expressions} in the statement
in bottom-up order and apply to each {\em expression}
the following algorithm:

\centerline{\bf Special Expression Algorithm}
\begin{enumerate}

\item If the {\em expression} is a {\em constant}\pagnote{CONSTANTS}, evaluate
the {\em expression} to a {\tt const} value $C$, and replace the
{\em expression} by `{\tt *QUOTE* \{ $C$ \}}',
where {\tt *QUOTE*} is a builtin function that
just returns its operand as its value, and terminate this algorithm.
...
has $N$ {\em assignment-results}, then the execution must return at
least $N$ results.  Assign these in order to the {\em assignment-results}
(excess results are discarded), and terminate this algorithm.

Otherwise replace the {\em function-call}
by `{\tt *QUOTE* $C$}' and terminate this algorithm.
...
Otherwise execute in the compiler the function defined by the
{\em function-declaration} and replace the entire
