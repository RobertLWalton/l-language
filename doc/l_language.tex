...
{\em reference-declaration} is found, it is a compile-error.
If a {\tt load} or {\tt store} declaration is found be the matching
{\em reference-offset} is not at the very end of $E$, it is a
compile error.
Note that to get a match, the type of the
{\em reference-base} \underline{cannot} be virtual.
...
Note that if $E$ is a load {\em reference-expression} naming a
variable that cannot be read (e.g., an {\tt *INIT*} variable),
or $E$ is a store {\em reference-expression} naming a variable
