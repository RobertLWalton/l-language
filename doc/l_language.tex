...
to \underline{out-of-line} functions.  The rules for assigning
depth to {\em statements} are as follows:
\begin{indpar}[0.2in]
\begin{tabular}{lp{5.0in}}
\key{(D1)}	& A {\em statement} that is not inside any other statement
                  (i.e., is not in a sub-block) is assigned depth 1.
\\[1ex]
\key{(D2)}	& A {\em statement} that is inside a sub-block at the end
                  of a {\em statement} of depth $N$ is given depth $N+1$.
\\[1ex]
\key{(D3)}	& An \underline{inline} {\em function-call} is treated
                  as creating a sub-block of the {\em statement}
		  containing the {\em function-call}.  Argument memory
		  levels are those of associated actual argument
		  memory levels.
\\[1ex]
\key{(D4)}	& {\em Statement} in the sub-block at the end of an
                  \underline{out-of-line} function declaration (and not
		  in any sub-block of that sub-block) are
		  assigned depth 1.
\\[1ex]
\key{(D5)}	& Arguments of an \underline{out-of-line} function
		  are given depth 0 within the sub-block of the
