...
overrides these functions: see \pagref{.@}.
...
Calls to {\tt .*} are {\em reference-calls}, so matching is bottom-up
and not top-down, and only arguments are matched,
not result types.\pagnote{REFERENCE-CALL-PROTOTYPE-MATCHING}
Thus the inserted calls, which have no
module abbreviation, can be matched only if there
is at most one {\tt ".*"} function definition in the current context
with suitable qualifiers and type (e.g., {\tt Q\$1} and {\tt T\$1})
for a given $P1$.
But you can provide variable definitions of {\tt .*} with diffenent actual
...
suitable qualifiers and type (e.g., {\tt Q\$1} and {\tt T\$1})
for a given $P1$.
