...
\item {\tt *HEAP*} may be replaced by {\tt *LOCAL*} with depth
equal to that of the {\em statement} copying the pointer, provided:
\begin{enumerate}
\item the source of the copy is a {\em reference-expression} that
is nothing but a {\em pointer-variable} (i.e., the source is a stack
variable, which will itself be {\tt co} even if its target is not)
\item the other qualifiers on the destination of the copy include
{\tt co} or {\tt ro}
\end{enumerate}
