...
is \underline{not} an expansion, the current area is set to `{\tt main}'
...
the {\em area-labels} of different areas must be distinct.
...
with the \key{parent} of an expansion X being the the {\em type-declaration}
or expansion with smallest scope containing X.
This tree-graph is called an \key{expansion tree}.

If a defined type has an expansion that does \underline{not}
end in {\tt ***} or {\tt *EXTERNAL*}, that expansion must \underline{always}
be the last
expansion of the {\em type-declaration} compiled, so that in this case
the expansion tree of the defined type is required to be linear.
...
as it is computed at the point where the {\tt *ORIGIN* *SIZE*}
