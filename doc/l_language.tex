...
\subsubsubsubsection{Defined Type Expansions }
...
\begin{indpar}\begin{verbatim}
pointer type dp is std data for dp
pointer type ap is std data for ap
...
is allowed within a subblock of the {\em block-assignment-statement}
...
The \key{lifetime} of a piece of memory is the time interval from
the time that the memory is allocated to the time that
the memory is deallocated.  The compiler tracks lifetimes by assigning each
pointer target a lifetime type and in some cases a separate lifetime depth.
...
\\[1ex]
\ttkey{*LOCAL*}		& the lifetime starts when the statement allocating
                          the memory executes and stops when the block
			  containing the statement terminates
\\[1ex]
\ttkey{*HEAP*}		& the lifetime starts when the memory is allocated
                          to the heap (i.e., garbage collectible
			  memory), and stops when the memory can no longer
			  be referenced by following a chain of pointer values
			  that are stored in memory that
			  has {\tt *GLOBAL*} lifetime
...
\key{(L2)}	& A pointer to a target with {\tt *HEAP*} lifetime type
                  (i.e., pointer target qualifier) can be stored in any value.
\\[1ex]
\key{(L3)}	& The {\tt *GLOBAL*} lifetime type
                  (i.e., pointer target qualifier) can be
...
\\[1ex]
		& Note: This requires the garbage collector to be able
		  ...
		  differently.
\\[1ex]
\key{(L4)}	& A pointer to a target with {\tt *LOCAL*} lifetime type
                  can\underline{not} be stored in a value pointed at by
		  a pointer with either {\tt *GLOBAL*} or {\tt *HEAP}
		  lifetime type.
\end{tabular}
\end{indpar}

The rule on when a pointer to a target with {\tt *LOCAL*}
lifetime type can be stored in a pointer target of {\tt *LOCAL*} lifetime type
references to the lifetime depth of values, which is defined below.
The rule is:

\begin{indpar}[0.2in]
\begin{tabular}{lp{5.0in}}
\key{(L5)}	& A pointer to a value $v1$ of lifetime type {\tt *LOCAL*}
...
\\[1ex]
		& Here $v2$ can be either the target of a pointer of
                  of lifetime type {\tt *LOCAL*} and lifetime depth $d2$,
		  or it can be a variable allocated by a
		  {\em block-variable-declaration}
		  or {\em next-variable-declaration} and being written
		  written inside the subblock of the associated
		  {\em block-assignment-statement} or
		  {\em loop-assignment-statement}.
\end{tabular}
\end{indpar}

\key{Lifetime depth} is assigned to a pointer stored in a {\tt co}
variable created by a
{\em result-variable-de\-clar\-a\-tion},
{\em block-variable-declaration},
or {\em next-variable-declaration} of an {\em assignment-state\-ment},
or a {\tt co} argument created by a {\em prototype-argument-declaration}
of a {\em function-call} or {\em reference-call}.

In this section `variable' means a variable as specified in the last
paragraph and `argument' means an argument as specified in the last
paragraph.

Lifetime depth of a variable is the same as the \key{statement depth}
of the {\em assignment-statement} creating the variable
(i.e., the {\em statement} that declares the variable).
...
\key{(S2)}	& A {\em statement} that is inside a sub-block at the end
		  ...
		  inside a sub-block of that sub-block, is given depth $D+1$.
\\[1ex]
...
\\[1ex]
\key{(S4)}	& A {\em statement} in the sub-block $S$ at the end of an
                  \underline{out-of-line} function declaration that itself
		  is a {\em statement} of depth $D$, and also not
		  in any sub-block of $S$, is assigned depth $D+1$.
\end{tabular}
\end{indpar}

The rules for assigning depth to arguments and results of \underline{inline}
{\em function-calls} and \\
{\em reference-calls} are:
\begin{indpar}[0.2in]
\begin{tabular}{lp{5.0in}}
\key{(I1)}	& An argument of {\tt *LOCAL*} pointer type
                  within an inline {\em function-prototype}
                  invoked by a
                  {\em function-call} is assigned
                  the same depth as the corresponding actual call argument.
\\[1ex]
\key{(I2)}	& Results of {\tt *LOCAL*} pointer type
                  of a inline {\em function-prototype} invoked by a
                  {\em function-call} are assigned
                  the same depth as the {\em statement} containing the
		  {\em function-call}.
\\[1ex]
\key{(I3)}	& An argument of {\tt *LOCAL} pointer type
                  within a {\em reference-function-prototype}
                  invoked by a
                  {\em re\-ference-call} is assigned
                  the same depth as the corresponding actual call argument.
\\[1ex]
\key{(I4)}	& A result of {\tt *LOCAL*} pointer type
                  of a {\em reference-function-prototype}
		  invoked by a
                  {\em reference-call} is assigned
		  the same depth as the {\em reference-expression}
		  that is the actual argument in the call
		  corresponding to the
		  {\em reference-pattern-argument} in the prototype.
\end{tabular}
\end{indpar}

(I4) is generalized to include {\em reference-expressions} that
are builtin.

\begin{indpar}[0.2in]
\begin{tabular}{lp{5.0in}}
\key{(I5)}	& A result of {\tt *LOCAL*} pointer type
                  of a {\em reference-expression} $E$ that is
		  \underline{not} a {\em reference-call} is assigned
		  the same depth as the {\em reference-expression}
		  that is the base of $E$.
\end{tabular}
\end{indpar}

TBD

The rules for assigning depth to variables are:
\begin{indpar}[0.2in]
\begin{tabular}{lp{5.0in}}
\key{(V1)}	& A pointer variable set by the `{\tt local}' allocator
                  must have the {\tt *LOCAL*} lifetime and is given
		  the depth of the statement that sets the variable.
\\[1ex]
\key{(V2)}	& A pointer variable with {\tt *LOCAL*} lifetime may be
                  set from the value of a pointer variable X
		  with {\tt *LOCAL*} lifetime and will be given
		  the depth of X.
\\[1ex]
\key{(V3)}	& A pointer variable with {\tt *GLOBAL*} or {\tt *HEAP*}
                  lifetime is given depth 0.
\\[1ex]
\key{(V4)}	& A pointer variable with {\tt *LOCAL*} lifetime may be
                  set to a pointer with {\tt *GLOBAL*} lifetime and will be
		  given depth 0.
\\[1ex]
\key{(V5)}	& A pointer variable with {\tt *LOCAL*} lifetime may be
                  set from the value of a pointer variable
		  with {\tt *HEAP*} lifetime and will be given depth 0.
\\[1ex]
		& Note: the pointer variable with {\tt *HEAP*} lifetime
		  will exist as long as the pointer variable with
		  {\tt *LOCAL*} lifetime exists.
\end{tabular}
\end{indpar}
...
values $v2$ pointed at by \underline{out-of-line} function arguments.
...
// Circular list:
//
ap *READ-WRITE* @last      // `=@ local = *DEFERRED*' is implied
    // First element is pointed at by last.@after
    // This variable has depth 1
...
function add ( ap *READ-WRITE* @element ):
    ...
    if @last is null:
        ...
        element.@after = last.@after
        last.@after = @element
    @last = @element

// Put two elements in list.
//
list element @X      // `=@ local = *DEFERRED*' is implied
                     // X memory is depth 1
list element @Y      // `=@ local = *DEFERRED*' is implied
                     // Y memory is depth 1
