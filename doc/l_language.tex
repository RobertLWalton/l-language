...
The type of a reference pointer\label{REFERENCE-POINTER-TYPE}
...
If one found, evaluate the declaration's function to {\tt const} return values.
...
(those annotated with a {\tt const} value), run-time
{\em reference-expressions},
and macros are processed before this step.
...
{\em reference-expression} that is annotated with its type,
which is the right-side {\em expression's} target type.
