...
\ttkey{*LOCAL*}		& The lifetime starts when the statement allocating
                          the variable executes and stops when the block
...
\ttkey{*HEAP*}		& The lifetime starts when the variable is allocated
			  ...
			  that are stored in variables that
			  have {\tt *GLOBAL*} lifetime
			  or have a {\tt *LOCAL*} lifetime that has not
			  yet terminated.
\end{tabular}
\end{indpar}
...
\\[1ex]
		& Note: This requires the garbage collector to be able
		  ...
		  differently.
\\[1ex]
\key{(L4)}	& A {\tt *LOCAL*} pointer
                  can\underline{not} be stored in
		  {\tt *GLOBAL*} or {\tt *HEAP} variable.
\end{tabular}
\\[1ex]
Note that for (L1) and (L2) the variable in which the pointer is stored
must have the same pointer type as the pointer value being stored.
These rules do not permit type conversion.
\end{indpar}
...
// Circular list:
//
ap *GLOBAL* *READ-WRITE* @last = null  // `=@ global' is implied
...
The rule on when a {\tt *LOCAL*}
can be stored a {\tt *LOCAL*} variable
...
// default to *LOCAL*.  Also, non-extermal assignment-variables
// of top level statements have depth 1.

// Circular list:
//
ap *READ-WRITE* @last = null  // `=@ local' is implied
    // First element is pointed at by last.@after
