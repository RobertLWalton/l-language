...
\begin{itemize}
\item The parenthese surrounding a {\em reference-expression}
may be implied.
\item An {\em expression} that does \underline{not} begin with
an explicitly or implicitly parenthesized subexpression or a
{\em module-abbreviation} followed by such a subexpression is
either a {\em reference-expression}
beginning with a {\em variable-name}\pagnote{VARIABLE-NAME}
or `\TT{next} {\em variable-name}'
or a {\em function-call}\pagnote{FUNCTION-CALL}
beginning with a {\em function-term-name} possibly
preceeded by a {\em module-abbreviation}.
\item
An {\em expression} that begins with a {\em variable-name}
or `\TT{next} {\em variable-name}'
or an explicitly or implicitly parenthesized {\em reference-expression}
possibly preceeded by a {\em module-abbreviation}
