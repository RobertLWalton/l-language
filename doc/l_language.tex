...
1 and falseif 0: it is in essence a 1-bit unsigned integer.
...
const c1 = 100
const c2 = 1000
int r6 = max ( x, c1 + c2 ) // OK; int is target type of + which
                            // has const result type
\end{verbatim}\end{indpar}
...
\begin{indpar}\begin{verbatim}
function ap *READ-WRITE* int = foo ( ... )
ap ro int * p = ro ( foo ( ... ) )
\end{verbatim}\end{indpar}
...
\hspace*{-0.2in}{\tt T v \TT{=} x + y * z} \\
       ...
       `{\tt x + y * z}' and thus must be the type of the prototype
       ...
       and {\tt z}.  Implicit conversions of variables is allowed,
...
\hspace*{-0.2in}{\tt x \TT{<} y \TT{<} z} \\
      ...
      If a comparison evaluates to {\tt TRUE} it is eliminated.
...
A \emkey{number-constant} is an {\em integer}, {\em number},
...
\emkey{decimal-constant-string} :::=
    \TT{"}~ {\em decimal-integer}~ {\em decimal-fraction}\QMARK{}~ \TT{"}
\\[0.5ex]
...
\emkey{binary-constant-string} :::=
    \TT{"}~ {\em binary-integer}~ {\em binary-fraction}\QMARK{}~ \TT{"}
\\[0.5ex]
...
\emkey{hexadecimal-constant-string} :::=
    \TT{"}~ {\em hexadecimal-integer}~ {\em hexadecimal-fraction}\QMARK{}~
    \TT{"}
\\[0.5ex]
...
Decimal integers may have commas
...
For decimal constants the denominator is a power of \TT{10}; for
binary constants the denominator is a power of \TT{2}; and for
hexadecimal constants the denominator is a power of \TT{16}.
...
\emkey{constant-expression}\label{CONSTANT-EXPRESSION}
    ::= {\em expression} that evaluates to a {\tt const} value
\\[0.5ex]
...
By default, dictionary members are marked read-only when they are created.
...
results or 0 results.  The types of the function prototype cannot
...
the data type of the {\em pointer-variable} being set (but the
...
function ap Q$1 file r = *ACCESS* *POINTER* *CONVERSION*
        ( fd Q$1 file p ):
...
The {\em default-values} are compiled in the contest of the prototype
...
it were with the left side consisting of the call's target type
...
but that companion may have the {\tt *EXPANABLE*} or {\tt *EXTERNAL*}
...
{\em Type-declarations} do not hide each other; instead the second
declaration expands (appends sub-declarations to)
...
is reserved when an out-of-line function is entered.  At this
time space is reserved for each of 4 or more iterations of
each loop, and the loop cycles between these iteration spaces.
Asside from loops reusing iteration spaced, there is no
freeing of these variables until the out-of-line function
...
Pointers to the heap (heap pointers) use the stub-body concept:
the heap pointer data points not at the body of a heap datum, but instead at a
...
pointer is read into a stack variable, or alternatively when
a heap pointer is written into a heap datum.

We will discuss in detail three kinds of garbage collection (GC):
copying, marking, and reference counting.  All three kinds
run interleaved with non-GC execution.
...
stub points at itself.  The during GC the body will be
...
points at a body pointer that points to new space, the
heap pointer is simply replaced by the body pointer.
...
the GC updates all pointers in the stack, and if now
new bodies are moved to new space, GC is done; otherwise
GC resumes scavanging.  Heap pointers in the stack are
not generally updated, and no special action is taken
...
efficient.  A disadvantage is that to end the GC
...
of just the body pointer without changing the old body pointer,
which is left pointing at inaccessible memory.
The update operation must do extra work if deallocated
...
compaction that is independent of GC.  The advantage
is that there is less body copying and less memory
space is required.  Also deallocated bodies do not
require that the update operation does extra work.

The simplest marking GC uses stubs that begin with
a body pointer followed by two list pointers,
a marked flag, and a scavenged flag.  The stubs are
...
the stub and there is just one stbu list.  The
marked and scavenged flags are used as befoe.
...
In reference counting GC each stub has a reference
...
in the stack or in a body.

Bodies may or may not be allocated separately from stubs.
If separate, deallocated bodies may be implemented and
...
5 subtypes: numbers, rationals, strings,
