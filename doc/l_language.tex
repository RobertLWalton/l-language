...
in the current function frame or in a module memory.  Other
...
Reference expressions differ from non-reference expressions in how
their result types are computed.  Non-reference expressions use
target types as their result types.  Reference expressions use the
type of their first operand, which is a {\em variable} or a shorter
...
on page \pageref{REFERENCE-FUNCTION-EXAMPLE},
...
Arithmetic subexpressions and function argument lists are bracketed
...
and prototype.  The {\em reference-call} ends just before the first
failed match, or when there are no more {\em call-argument-lists}.
