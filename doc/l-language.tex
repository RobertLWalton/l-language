...
\key{indirect variable} associated to {\tt @V}
...
A {\em reference-expression} begins with a {\em variable-name}.
If the variable is declared to have a pointer value,
the {\em reference-expression} computes a pointer.
When the {\em reference-expression} selects a member or element,
a pointer to the member or element is designated.

A variable whose name begins with `{\tt @}' and which names a pointer
value has an associated \key{indirect variable} whose name is made
by removing the initial `{\tt @}' from the variable name (e.g.,
the indirect variable associated with {\tt @V} is {\tt V}).  The
associated indirect variable names the value pointed at.
When a {\em reference-expression} beginning with an indirect
variable selects a member or element,
the value of the member or element is designated.

A variable that is not indirect is called a \key{direct variable}.
When a {\em reference-expression} beginning with an direct
variable selects a member or element, if the direct variable names
a value that is not a pointer,
the value of the member or element is designated, but if the
direct variable names a value that is a pointer, a pointer to the
member or element is designated.
...
There are no compile-time pointers analogous to {\tt ap}.
