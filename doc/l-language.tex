...
there are {\em control-statements}
...
and zeros that memory.  Then any {\em statements} and
{\em exit-subblocks} are executed with
the allocated memory being write-only.

More specifically, if there is no {\em allocation-call},
the {\em variable-declaration} variables are allocated to the
current function frame and zeroed.

If there is an {\em allocation-call}, the {\em allocation-call} will be
executed with a pre-pended argument list separately for each
{\em variable-declaration}.  The declared variable must have pointer
