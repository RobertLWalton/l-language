...
A run-time variable that is not indirect is called a
\key{direct variable}\label{DIRECT-VARIABLE}.
...
\hspace*{1.0in}
        {\em qualifier-name}\STAR{} {\em type-name} {\em direct-variable-name}
\\[0.5ex]
\emkey{next-variable-declaration}\label{NEXT-VARIABLE-DECLARATION}
    ::= \ttkey{next} {\em direct-variable-name}
\\[0.5ex]
...
\\[0.5ex]
\emkey{direct-variable-name} ::= {\em variable-name} of a direct variable
				  (\pagref{DIRECT-VARIABLE})
\\[0.5ex]
\emkey{variable-name} ::= see \pagref{VARIABLE-NAME}
\\[0.5ex]
...
function data for ap r = pointer to data ( ap Q$1 T$1 ptr )
    ...
    // result value changing the type of the value.

// Implicitely convert a dp to an ap whose base is the
// constant 0 and whose offset is the dp.
// 
function ap Q$1 T$1 r = *IMPLIED* *CONVERSION* ( dp Q$1 T$1 ptr ):
    int ptr data = pointer to data ( ptr )
    ...
    data for ap dap:
        dap.@base  = @zero
        dap.offset = ptr data
    r = *UNCHECKED* ( data for ap )

// Unchecked conversion of an ap to a dp that is the sum of
// the base and offset of the ap.
//
function dp Q$1 T$1 r = *UNCHECKED* ( ap Q$1 T$1 ptr ):
    data for ap dap = pointer to data ( ptr )
    int ptr = dap.base + dap.offset
    r = *UNCHECKED* ( ptr )
\end{verbatim}\end{indpar}

In the above {\tt Q\$1} is a qualifier wild-card that matches
any list of {\em qualifier-names}, and {\tt T\$1} is a type wild-card that
matches any {\em type-name}.
...
pointer type, an {\tt *UNCHECKED*} function
from the defined pointer type to a buitin pointer type must
be declared.
