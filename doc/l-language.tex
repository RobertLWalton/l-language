...
must be in the same modules (\pagref{MODULE-DECLARATIONS}) or its
bodies.
...
and if the {\em type-name} is external (\pagref{SCOPE}),
...
with the same {\em type-name} in different modules and bodies
(\pagref{MODULE-DECLARATIONS}),
...
deficient is accepted.  All other cases are a compiler error.
...
{\em function-call} (\pagref{FUNCTION-CALL}).  A foreign
...
is external, its companion may be in the body of a module
...
is external if the {\em prototype-pattern} in the {\em declaration}
...
with the exception of some {\em type-declarations}
(see \pagref{*EXTERNAL*}) and some companion
{\em out-of-line-function-declarations}
(see \pagref{OUT-OF-LINE-EXTERNAL-COMPANION}).
...
Hiding of this kind is a compiler error.
...
or if it is a {\em deferred-result} (\pagref{DEFERRED-RESULT}).
...
{\em Type-declarations} cannot hide each other.  The second
declaration extends the first.
