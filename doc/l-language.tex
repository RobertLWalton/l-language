...
A constant pointer value for a function type can be declared by:
\begin{indpar}
\emkey{function-value-declaration}\label{FUNCTION-VALUE-DECLARATION} ::= \\
\hspace*{0.5in}
    \begin{tabular}[t]{rl}
        &  {\em function-type-name} {function-value-name} \TT{:} \\
	& \TT{~~~~~}{\em statement}\PLUS{} \\
    \end{tabular} \\
\emkey{function-value-name} ::=
    {\em module-abbreviation}\QMARK{} {\em basic-name}
\end{indpar}

Here the first line functions as an {\em out-of-line-function-prototype}
made by taking the {\em function-type-prototype} from the
{\em function-type-name} and replacing the {\tt ()}
{\em out-of-line-function-name} by the {\em function-value-name}
while adding `{\tt out-of-line}' to its beginning.  In addition
the {\em function-value-name} is declared as a variable whose
value has the {\em function-type-name} as its {\em type-name}.
