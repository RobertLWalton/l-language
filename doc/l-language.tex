...
More specifically, if there is no {\em function-call} and no
`{\tt deferred}', the
...
the variables are allocated as before, but {\em statements} to be
executed with the variables being treated as {\tt *WRITE-ONLY*} are
deferred to subsequence {\em allocation-statements} for (one or more of)
the same variables.  These subsequent {\em statements} must occur
within the scope (see \itemref{SCOPE}) of the deferred {\em statement},
and if a variable is external, all these subsequent statements must
...
these {\em statements}, the qualifiers of these variables, and of the members
and elements of their values, and of the members and elements of the members and
elements, and so forth recursively, are effectively changed to
{\tt *WRITE-ONLY*}.
