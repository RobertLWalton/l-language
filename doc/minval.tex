...
aligned, {\tt weight} has offet 8).  A structure type has an
alignment equal to the greatest common multiple of the
alignments of its members.  Structure types can be extended
...
However a value of a structure type is not actually the
structure proper, but instead it is a pointer to the structure,
stored in a machine word.  Thus in `{\tt X = ...}' the {\tt xxx}
must be a function call that allocates the structure.  However,
the keyword {\tt allocate} as used in the example causes the
structure to be allocate inline, in this case in the current
function execution frame.  The value of {\tt Y} is still a pointer to this
structure, and is stored in a different part of the function
frame from the structure itself.
