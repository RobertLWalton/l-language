...
User defined types consist of a sequence of bytes containing fields.
...
In this example there is one the field in a {\tt my type} value
that is an unlabeled 32 bit unsigned integer, that is, an {\tt uns32}
value.  Inside this field there are 6 subfields, the first of which is
an {\tt uns8} integer occupying bits 0-7 of the first field,
where bits are numbers 0, 1, 2, \ldots{} from
low to high order.  The second subfield is a 1-bit {\tt bool}
value that occupies bit 0 of its containing field.

Defined type values are alignment on byte boundaries when
...
Fields can be packed to aligned: here they are initially packed
...
A {\tt label} is like a zero length member that has no value, and is used to
position other members via the {\tt origin} statement.
The {\tt include} statement copies all the substatements
from another user defined type.

Here {\tt extension} is aligned on a 64 bit boundary.
A defined type has an
alignment in bytes equal to the greatest common multiple of the
