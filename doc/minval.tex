...
in the example), and this sequence is interpreted as an
{\tt int}, {\tt uns}, or {\tt bool},
and is given
a name ({\tt op\_code}, {\tt has\_constant}, etc.~in the
example).

Notice that fields can overlap and a variable can be
defined as having a location inside a packed number
({\tt Y} in the example).
...
The {\tt include} statement copies members, {\tt align},
and {\tt origin} declarations from another structure type.
...
Pointers can point at structures and structure members, and
such pointers can be assigned to
variable locations (as in {\tt @ V = {\em pointer-\EOL value}}).
