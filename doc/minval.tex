...
\\[1ex]
where {\em reference-expressions} are \underline{not} allowed on the
{\em assignment-left-side}
\end{indpar}
...
from the associated {\em assignment-result}).

A block may be exited by a `{\tt go to {\em block-label} exit}'
statement within the block.  A {\em exit-subblock} of a block may be entered
by a `{\tt go to {\em exit-label} exit}' statement within the block.
Without such a statement {\em exit-subblocks} are not executed,
and if execution gets to the end of an {\em exit-subblock},
the entire block is exited.  However an {\em exit-subblock}
may contain a `{\tt go to \ldots{}~exit}' statement that goes to
a \underline{subsequent} {\em exit-subblock}.  There are
variable scoping requirements that must be met when using
{\tt go to \ldots{}~exit} statements: see \itemref{SCOPING}.
