...
{\em function-calls} are not allowed outside assembly-time
{\em expressions}).
\item For run-time {\em expression-assignment-statements} with
a \underline{single} right-side {\em expression}, this {\em expression}
may be any run-time {\em expression} (i.e., a single non-prefix
operator, some prefix-operators depending on context, and a single
run-time {\em function-call} is allowed).
\end{itemize}
\end{indpar}

All the right-side {\em expression} values and locations are
evaluated first, all left-size {\em assignment-result} locations
are evaluated next if the {\em assignment-operator} is {\tt =}
or {\tt =@}, and then the {\em assignment-operator} is applied
to each matching {\em assignment-result} and {\em expression}.

There is an exception to this if the right-side is a single
{\em function-call}, as a {\em function-call} can return more
than one result.  In this the {\em function-call} must return
at least as many results as there are {\em assignment-results}
and all only the {\em assignment-primary-result} is returned
by the {\em assignment-operator}, with all the 
{\em assignment-secondary-results} being returned by the {\tt =}
operator.

Note that there are other semantic rules such as the following:
no {\em reference-expression} may be an {\em assignment-result}
of a {\tt @=@} or {\tt @=} operator.
