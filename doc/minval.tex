...
A {\em prototype-pattern} with the syntax:
\begin{indpar}
\emkey{boolean-prototype-pattern} ::=
...
If a {\em function-term-name} in the prototype does not match
any {\em call-term-name}, any {\em pattern-argument-lists} in the
{\em pattern-term} of the {\em function-term-name} must have
\TT{(~)} brackets, may \underline{not} have \TT{[~]} brackets,
and must have {\em default-values} in all their
{\em argument-declarations}.
...
Arguments if a {\em call-argument-list} is shorter than the
