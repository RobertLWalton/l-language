...
Assembly time data or just the {\tt const} values.
There are 4 types of {\tt const} values: integers, rationals, strings,
and maps.

\key{Integers} are unbounded and can be arbitrarily long.
Any sequence of lexemes that evaluated to a number constant
that is integral can be used to represent an integer {\tt const}.
...
Any sequence of lexemes that evaluate to a number constant that
is not integral can be used to represent a rational {\tt const}.
E.g., {\tt 1e-2} is the same as {\tt 1 / 100}.
...
been moved to the beginning of the list.
