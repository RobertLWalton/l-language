...
These variables have locations which designate the
...
low order bits contain the size of the field in bits minus 1.
\item A `lower bound' {\tt int} which is the minimum allowed
value of the offset {\tt int} (assuming the filed size does not
change).
...
\end{itemize}
Fields can be signed or unsigned integers with sizes from 1 through
the size of an {\tt int} (which is 32 or 64 bits depending on
the computer architecture).

There are other types of pointer.  A {\tt nv} is like a field
vector except that the offset is in bytes and the number type
must specify the size of the number (e.g., {\tt int8}, {\tt flt64}
for 64-bit IEEE floats).  The {\tt fp} and {\tt np} types are
similar but do not have the bounds and cannot be indexed.  There
is also a direct pointer, {\tt dp}, that is just a single {\tt int}
...
in module memory (perhaps a different module than the current
...
Then the location of {\tt Y} is, more specifically, an {\tt fp} field pointer 
